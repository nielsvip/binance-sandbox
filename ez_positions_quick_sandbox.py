...
from config_sandbox import Config
from ez_manage_sandbox import (MultiAccountTradeManager, OrderQueue,
                               _last_events_cache_time, generate_unique_id,
                               load_accounts, minutes_since,
                               verify_trade_via_websocket)
from ez_positions_service_sandbox import bootstrap_position_service
...
class AccountConfig:
    ...
    _last_time_sync: float = field(default=0.0, init=False)
...
async def safe_check_server_heartbeat(account_key: Optional[str] = None) -> bool: return await check_server_heartbeat(account_key)
...
def calculate_dynamic_quantity(symbol: str, current_price: float, score: int, config_obj, trade_manager, tracker_data: Dict[str, Any] = None, is_long: bool = True, indicators: Dict[str, Any] = None, metrics: Dict[str, Any] = None, curr_amt: float = 0.0, account_key: str = None) -> float:
    ...
    if trade_manager and hasattr(trade_manager, 'positions_service') and getattr(trade_manager, 'positions_service', None) and account_key:
        try:
            ...
            if is_long:
                if short_pct > 55.0:
                    skew_factor = (short_pct - 50.0) / 10.0
                    ratio_mult = 1.0 + skew_factor
                elif long_pct > 60.0:
                    ratio_mult = max(0.2, 1.0 - (long_pct - 60.0) / 20.0)
            else:
                if long_pct > 55.0:
                    skew_factor = (long_pct - 50.0) / 10.0
                    ratio_mult = 1.0 + skew_factor
                elif short_pct > 60.0:
                    ratio_mult = max(0.2, 1.0 - (short_pct - 60.0) / 20.0)
        except Exception:
            pass
    if ratio_mult == 1.0:
    ...
    dc_score = 0.0
    mtf_tfs = ['1m', '3m', '15m', '1h', '4h', 'D']
    for tf_mtf in mtf_tfs:
        h_mtf = safe_fetch_float(indicators.get(f'dc_high_{tf_mtf}'), 0)
        l_mtf = safe_fetch_float(indicators.get(f'dc_low_{tf_mtf}'), 0)
        if h_mtf > l_mtf:
            pos_pct_mtf = (current_price - l_mtf) / (h_mtf - l_mtf)
            dc_score += (1.0 - pos_pct_mtf) if is_long else pos_pct_mtf
    mtf_dc_mult = 0.5 + (dc_score / len(mtf_tfs))
    mtf_dc_mult = max(0.4, min(3.0, mtf_dc_mult))
    ...
    if score >= 25 or (tracker_data and "BRING_OUT_THE_ARMY" in str(tracker_data.get('last_reason', ''))):
        ...
        dc_mult = 1.0
        value_mult = 1.0
    target_notional = base_usdc_size * mode_mult * ratio_mult * alpha_mult * score_mult * history_mult * crash_mult * value_mult * dc_mult * knife_penalty * mtf_dc_mult
        ...
        def safe_float(x):
            ...
            if isinstance(x, dict):
                for k in ("amount", "qty", "positionAmt", "value"):
                    if x and k in x: return safe_float(x[k])
            try: return float(x)
...
class AdvancedSignalRater:
    ...
    async def rate(account_key, symbol, is_long, current_price, metrics, ind, prev_cross_price, is_exit, is_allowed, avg_entry=0.0, last_exit_timestamp=None, last_reduction_price=0.0, scalping_mode=False, scalping_override=False, tracker_data=None, tracker_manager=None):
        ...
        # Indicators >15 min old are NEVER allowed in any entry calculation.
        # Exits are still allowed (we always want to be able to get out).
        if not is_exit and not is_hedge:
            try:
                _ts_check = ind.get('timestamp_15m') or ind.get('timestamp_3m') or ind.get('timestamp', '')
                if _ts_check:
                    _ts_dt = datetime.fromisoformat(str(_ts_check).replace('Z', '+00:00'))
                    _age_min = (datetime.now(timezone.utc) - _ts_dt).total_seconds() / 60.0
                    if _age_min > 25.0:
                        logger.error(f"[rate] {position_key}: ❌ BLOCKED - STALE_INDICATORS ({_age_min:.0f}m old, ts={str(_ts_check)[:19]})")
                        return 0, "WAIT", f"STALE_INDICATORS_{_age_min:.0f}m"
            except Exception: pass
        # ─────────────────────────────────────────────────────────────────────

        # BASIS_CONDITION BOYCOTT
        if getattr(config, 'BASIS_CONDITION', False) and not is_exit:
        ...
        # HTF TREND SCORING — 1h, 4h, D stoch + heikin ashi + dc_basis + sma_200
        htf_bull = 0; htf_bear = 0
        if k_1h > d_1h: htf_bull += 1
        else: htf_bear += 1
        if k_4h > d_4h: htf_bull += 1
        else: htf_bear += 1
        if ha_1h == 'green': htf_bull += 1
        elif ha_1h == 'red': htf_bear += 1
        if ha_4h == 'green': htf_bull += 1
        elif ha_4h == 'red': htf_bear += 1
        if ha_D == 'green': htf_bull += 1
        elif ha_D == 'red': htf_bear += 1
        if dc_basis_1h > 0 and current_price > dc_basis_1h: htf_bull += 1
        elif dc_basis_1h > 0 and current_price < dc_basis_1h: htf_bear += 1
        if dc_basis_4h > 0 and current_price > dc_basis_4h: htf_bull += 1
        elif dc_basis_4h > 0 and current_price < dc_basis_4h: htf_bear += 1
        if sma_200_1h > 0 and current_price > sma_200_1h: htf_bull += 1
        elif sma_200_1h > 0 and current_price < sma_200_1h: htf_bear += 1
        htf_trend_score = htf_bull - htf_bear  # range roughly -8 to +8
        htf_trend_bullish = htf_trend_score >= 2
        htf_trend_bearish = htf_trend_score <= -2
        # HTF VETO — block entries against dominant higher timeframe trend
        if not is_exit and not is_hedge:
            if is_long and htf_bear >= 5:
                return 0, "WAIT", f"HTF_TREND_VETO_LONG_htfBull{htf_bull}_htfBear{htf_bear}_ha1h{ha_1h}_ha4h{ha_4h}_haD{ha_D}"
            if not is_long and htf_bull >= 5:
                return 0, "WAIT", f"HTF_TREND_VETO_SHORT_htfBull{htf_bull}_htfBear{htf_bear}_ha1h{ha_1h}_ha4h{ha_4h}_haD{ha_D}"
...
class HedgeEngine:
    ...
    async def scan_and_hedge_losers(self, account_key: str):
        ...
        for position_key, pos in positions.items():
            ...
            if pnl_pct < _hedge_trigger:
                ...
                if abs(pnl_pct) < 1.0:
                    logger.debug(f"[HEDGE_BLOCK_LOW_LOSS] {position_key}: loss {pnl_pct:.2f}% < 1.0% — no hedge needed, just close if it gets worse")
                    continue
                if notional < 50.0:
                    ...
                    for record in hedges:
                        try:
                            ...
                            if hedge_gain < hedge_prev_gain:
                                logger.critical(f"🚨 [HEDGE_TIDE_TURN_KILL] Hedge {hedge_key} FIRST TICK DOWN: {hedge_gain:.3f}% < prev {hedge_prev_gain:.3f}%. INSTANT KILL on first opposite tick.")
                                await execute_trade_wrapper( trade_manager=self.trade_manager, tracker_manager=self.tracker_manager, hedge_engine=self, account_key=account_key, position_key=hedge_key, positionAmt=hedge_amt, action='CLOSE', current_price=h_price, qty=hedge_amt, reason=f"HEDGE_TIDE_TURN_KILL_{hedge_gain:.3f}%<prev_{hedge_prev_gain:.3f}%", is_hedge=True, hedge_for=losing_key, data_manager=self.data_manager )
                                await self.tracker_manager.nuke_hedge_key(account_key, hedge_key)
                                continue
                            ...
                            if hedge_gain > 0.0 and hedge_gain < hedge_prev_gain:
                                logger.critical(f"🔥 [HEDGE_PROFIT_DECLINING] Hedge {hedge_key}: gain {hedge_gain:.3f}% < prev {hedge_prev_gain:.3f}%. FIRST TICK DOWN = INSTANT KILL.")
                                await execute_trade_wrapper(trade_manager=self.trade_manager, tracker_manager=self.tracker_manager, hedge_engine=self, account_key=account_key, position_key=hedge_key, positionAmt=hedge_amt, action='CLOSE', current_price=h_price, qty=hedge_amt, reason=f"HEDGE_PROFIT_DECLINING_{hedge_gain:.3f}%<prev_{hedge_prev_gain:.3f}%", is_hedge=True, hedge_for=losing_key, data_manager=self.data_manager)
                                await self.tracker_manager.nuke_hedge_key(account_key, hedge_key)
                                continue
                            ...
                            qty_to_trade = 0.0
                            if hedge_gain < 0.0:
                                logger.critical(f"🛑 [HEDGE_LOSING_KILL] {hedge_key} is losing {hedge_gain:.2f}% — killing hedge AND original {losing_key}")
                                await execute_trade_wrapper(trade_manager=self.trade_manager, tracker_manager=self.tracker_manager, hedge_engine=self, account_key=account_key, position_key=hedge_key, positionAmt=hedge_amt, action='CLOSE', current_price=h_price, qty=hedge_amt, reason=f"HEDGE_KILL_LOSING_{hedge_gain:.2f}%", is_hedge=True, hedge_for=losing_key, data_manager=self.data_manager)
                                await self.tracker_manager.nuke_hedge_key(account_key, hedge_key)
                                if losing_pos and losing_pnl < -0.3:
                                    side_kill = 'SELL' if losing_pos.position_side == 'LONG' else 'BUY'
                                    await self.trade_manager.execute_now(losing_key, account_key, losing_sym, losing_amt, side_kill, losing_pos.position_side, losing_amt, l_price, f"HEDGE_PAIR_KILL_{int(time.time())}", f"FORCE_HEDGE_PAIR_KILL_hedge_lost_{hedge_gain:.2f}%_orig_{losing_pnl:.2f}%", True, 'CLOSE')
                                continue
        ...
        for record in active_hedges:
            try:
                ...
                if hedge_gain < 0.0 or decay_kill:
                    reason = f"STRICT_LOSS_{hedge_gain:.2f}%" if hedge_gain < 0.0 else f"FIRST_TICK_DOWN_{hedge_prev_gain:.2f}%->{hedge_gain:.2f}%"; logger.warning(f"💀 [HEDGE_KILL] {hedge_key} {reason}. FIRST TICK = INSTANT KILL.")
    ...
    async def find_hedge_candidates(self, account_key: str, losing_symbol: str, losing_side: str) -> List[Tuple[str, float, str]]:
        ...
        for hottest_sym, registry_score in hottest_candidates:
            ...
            if price <= 0: continue
            hedge_score = await self._calculate_hedge_candidate_score(account_key, hottest_sym, target_side, price, losing_symbol)
            if hedge_score > 0.15:
    ...
    async def _add_dynamic_fallback_candidates(self, account_key, target_side, losing_symbol, exclude_list, existing_candidates):
        ...
        for sym, ratings in cache.items():
            ...
            if price <= 0: continue
            hedge_score = await self._calculate_hedge_candidate_score(account_key, sym, target_side, price, losing_symbol)
    ...
    async def execute_dual_hedge(self, account_key: str, losing_position_key: str, losing_symbol: str, losing_side: str, losing_value_usd: float, dry_run: bool = False) -> Dict[str, Any]:
        ...
        async with self._get_account_lock(account_key): 
            ...
            if not elected_success:
                ...
                logger.warning(f"[HEDGE_NOCANDIDATE] {losing_symbol}: No qualified elected candidate. Skipping self-hedge, reducing losing position.")
            ...
            if elected_success or actual_status == 'success':
                ...
                logger.critical(f"🚨 [HEDGE_CRITICAL] ALL Hedges failed for {losing_symbol}. Closing losing position IMMEDIATELY.")
                ...
                if losing_pos:
                    ...
                    current_price, _ = await self.data_manager.get_fresh_price(losing_symbol)
                    if current_amt > 0:
                        success, reason = await execute_trade_wrapper( trade_manager=self.trade_manager, tracker_manager=self.tracker_manager, hedge_engine=self, account_key=account_key, position_key=losing_position_key, positionAmt=current_amt, action='CLOSE', current_price=current_price, qty=current_amt, reason="HEDGE_FAILED_EMERGENCY_CLOSE", already_locked=False, is_hedge=False, override_qty=current_amt, data_manager=self.data_manager )
    ...
    async def _emergency_reduce_losing(self, account_key, losing_key, losing_symbol, losing_side):
        ...
        logger.warning(f"🛡️ [HEDGE_FALLBACK_REDUCE] No hedge candidates for {losing_key}. Reducing {reduce_qty:.6f} (40%) @ ${current_price:.4f}")
        side = 'SELL' if losing_side == 'LONG' else 'BUY'
        success, result = await execute_trade_wrapper(trade_manager=self.trade_manager, tracker_manager=self.tracker_manager, hedge_engine=self, account_key=account_key, position_key=losing_key, positionAmt=current_amt, action='REDUCE', current_price=current_price, qty=reduce_qty, reason="HEDGE_NO_CANDIDATE_EMERGENCY_REDUCE", already_locked=False, is_hedge=False, override_qty=reduce_qty, data_manager=self.data_manager)
    ...
    async def execute_same_symbol_hedge(self, account_key, origin_position, symbol, origin_side, qty, current_price):
        ...
        if positionAmt > 0:
            logger.warning(f"🛑 [HEDGE_SAME_ALREADY_EXISTS] {hedge_key} already open (amt={positionAmt:.4f}). Hedge = ONE entry only.")
            return True  # Already hedged, don't double up
    ...
    async def _scan_for_entries(self, account_key: str, rules: dict, target_long_ratio: float, snapshot: dict, global_score: float):
        ...
        for symbol, data in snapshot.items():
            ...
            if allow_long and current_divergence > 8 and div_delta_3m > 5:
                ...
                k_1m = safe_fetch_float(data.get('stoch_k_1m', 50))
                dc_low_15m = safe_fetch_float(data.get('dc_low_15m', 0))
                dc_high_15m = safe_fetch_float(data.get('dc_high_15m', 0))
                dc_range = dc_high_15m - dc_low_15m if dc_high_15m > dc_low_15m else 1.0
                dc_position = (current_price - dc_low_15m) / dc_range if dc_range > 0 else 0.5
                level_ok = k_3m < 40 or dc_position < 0.35
                tandem_ok = (k_1m < 50 and k_15m < 50) or (k_3m < 50 and k_15m < 50)
                if k_15m < 85 and level_ok and tandem_ok:
                    heavy = ha_setup and ha_dir == 'LONG'
                    bonus = (ha_score * 8) if heavy else 0
                    candidates.append({ 'symbol': symbol, 'side': 'LONG', 'score': current_divergence + div_delta_3m + bonus, 'price': current_price, 'reason': f"DIV:{current_divergence:.1f}_VEL3m:{div_delta_3m:.1f}_k3m:{k_3m:.0f}_dc:{dc_position:.2f}", 'ha_score': ha_score if heavy else 0, 'ha_detail': ha_detail if heavy else '' })
                ...
                dc_range = dc_high_15m - dc_low_15m if dc_high_15m > dc_low_15m else 1.0
                dc_position = (current_price - dc_low_15m) / dc_range if dc_range > 0 else 0.5
                level_ok = k_3m > 60 or dc_position > 0.65
                tandem_ok = (k_1m > 50 and k_15m > 50) or (k_3m > 50 and k_15m > 50)
                if k_15m > 15 and level_ok and tandem_ok:
                    heavy = ha_setup and ha_dir == 'SHORT'
                    bonus = (ha_score * 8) if heavy else 0
                    candidates.append({ 'symbol': symbol, 'side': 'SHORT', 'score': abs(current_divergence + div_delta_3m) + bonus, 'price': current_price, 'reason': f"DIV:{current_divergence:.1f}_VEL3m:{div_delta_3m:.1f}_k3m:{k_3m:.0f}_dc:{dc_position:.2f}", 'ha_score': ha_score if heavy else 0, 'ha_detail': ha_detail if heavy else '' })
...
class FastDataManager:
    ...
    async def _maintain_cold_data_sync(self):
        """Poll Redis key 'latest_market_data' every 10s; reload whenever data changes."""
                    ...
                    if client:
                        ...
                        if raw:
                            ...
                            if fp != last_fingerprint:
                                if self._apply_cold_payload(raw):
                                    last_fingerprint = fp
    ...
    async def get_hot_state(self, symbol: str):
        ...
        if self.shared_proxy:
            ...
            except Exception: self.shared_proxy = None 
...
class SentimentExposureManager:
    ...
    async def _manage_individual_position(self, position_key: str, is_bullish_regime: bool):
        ...
        if b15 > 0 and b1h > 0 and b4h > 0 and bd > 0:
            ...
            if violation and getattr(config, "BASIS_CONDITION", False) and pnl_pct >= 0.10:
                if await self.tracker_manager.is_trade_cooldown_active(position_key): pass
                else:
...
async def execute_trade_wrapper(trade_manager, tracker_manager: TrackerManager, hedge_engine: HedgeEngine, account_key: str, position_key: str, positionAmt:float, action: str, current_price: float, qty: float, reason: str, already_locked: bool = False, is_hedge: bool = False, hedge_for: Optional[str] = None, override_qty: Optional[float] = None, verify_via_websocket: bool = True, data_manager: Optional[FastDataManager] = None) -> tuple[bool, str]:
    ...
    if getattr(config, "LS_RATIO_ENFORCE", False) and ("OPEN" in action or "AUGMENT" in action) and not is_hedge:
        total_min_val = 50.0
    ...
    is_force_dc = 'FORCE_DC' in reason.upper()
    ...
    # SCALP MOMENTUM BOYCOTT
    scalp_accounts = getattr(config, 'SCALP_ACCOUNTS', [])
    if getattr(config, 'SCALP_MODE', False) and account_key in scalp_accounts and ('OPEN' in action or 'AUGMENT' in action) and not is_hedge and not is_force_dc:
    ...
    async def process_single_exit(position_key):
        async with sem:
            try:
                ...
                if not hard_exit_reason and not is_hedge and current_gain >= 0.10:
                    if max_gain > 0.5 and current_gain <= 0.20:
                        hard_exit_reason = f"BREAK_EVEN_GUARD_CLOSE_peak{max_gain:.2f}%_curr{current_gain:.2f}%"
                    elif max_gain > 1.5 and current_gain < (max_gain * 0.5):
                ...
                # CYCLE_TP: exit at 15m stoch cycle peak/trough while in profit
                # Don't sit through a full 15m down-cycle while in profit
                if not hard_exit_reason and not is_hedge and current_gain > 0.15:
                    if is_long and (k_15mcu or (k_15m > 75 and k_15m < k_15m_prev and ha_15m == 'red')):
                        hard_exit_reason = f"CYCLE_TP_15M_PEAK_{current_gain:.2f}%_k:{k_15m:.0f}"
                    elif not is_long and (k_15mco or (k_15m < 25 and k_15m > k_15m_prev and ha_15m == 'green')):
                        hard_exit_reason = f"CYCLE_TP_15M_TROUGH_{current_gain:.2f}%_k:{k_15m:.0f}"
                if not hard_exit_reason and not is_hedge and current_gain >= 0.10:
                    ...
                    elif (max_gain > 1.0 and erosion > 0.4):
                ...
                if not hard_exit_reason:
                    ...
                    elif (max_gain > 2.0 and (max_gain - current_gain) > 0.8):
        ...
        try:
            async with sem:
                ...
                if (score >= 4 or "BUY" in rec or "SELL" in rec) and ('WAIT' not in str(rec) and 'HEDGE' not in str(rec)):
                    should_trade = True
                if not should_trade and 'STALE_INDICATORS' not in str(reason) and position_key in tracker_manager.tradeable_position_keys.get(account_key, set()):
                    # --- DC TIER ENTRY (replaces old FORCE_DC spam) ---
                    # Only enter when price is MEANINGFULLY outside a DC channel (0.1% buffer).
                    # Higher TF = bigger conviction = larger position tier.
                    # Tiers: 3m=1×, 15m=1.5×, 1h=2×, 4h=3× (crypto uses 3m not 5m)
                    _force_fresh = False
                    try:
                        ...
                        if _ts3m:
                            ...
                            _force_fresh = (datetime.now(timezone.utc) - _ts_dt).total_seconds() < 1200
                    except Exception: pass
                    ...
                    if _force_fresh:
                        ...
                        _dc_tf, _dc_mult = None, 1.0
                        if is_long:
                            # Price above DC high = breakout LONG
                            if _dch4h > 0 and current_price > _dch4h * (1 + _buf): _dc_tf, _dc_mult = "DC4H", 3.0
                            elif _dch1h > 0 and current_price > _dch1h * (1 + _buf): _dc_tf, _dc_mult = "DC1H", 2.0
                            elif _dch15 > 0 and current_price > _dch15 * (1 + _buf): _dc_tf, _dc_mult = "DC15M", 1.5
                            elif _dch3  > 0 and current_price > _dch3  * (1 + _buf): _dc_tf, _dc_mult = "DC3M", 1.0
                        else:
                            # Price below DC low = breakout SHORT
                            if _dcl4h > 0 and current_price < _dcl4h * (1 - _buf): _dc_tf, _dc_mult = "DC4H", 3.0
                            elif _dcl1h > 0 and current_price < _dcl1h * (1 - _buf): _dc_tf, _dc_mult = "DC1H", 2.0
                            elif _dcl15 > 0 and current_price < _dcl15 * (1 - _buf): _dc_tf, _dc_mult = "DC15M", 1.5
                            elif _dcl3  > 0 and current_price < _dcl3  * (1 - _buf): _dc_tf, _dc_mult = "DC3M", 1.0

                        # Direction confirmation: ALL 3 stoch timeframes must agree
                        # Without this, entries fire on pullbacks within a channel break (dc stays for hours)
                        ...
                        if is_long:
                            _stoch_confirms = (_k1m > _d1m) and (_k3m > _d3m) and (_k15m > _d15m)
                        else:
                            _stoch_confirms = (_k1m < _d1m) and (_k3m < _d3m) and (_k15m < _d15m)

                        # Only trigger on 15m+ timeframes — 3m alone is noise
                        # AND stoch must confirm active momentum in the breakout direction
                        if _dc_tf and _dc_tf != "DC3M" and _stoch_confirms:
                            should_trade = True
                            score = max(score, 15.0 + _dc_mult * 5)
                            reason += f"_DC_TIER_{_dc_tf}"
...
async def aggressive_hedge_scanner(trade_manager, account_key: str, tracker_manager: TrackerManager, hedge_engine: HedgeEngine):
    if not is_hedge_account(config, account_key):
        return  # Only hedge on hedge-enabled accounts (inf, fin, men, flz — NOT ang)
    try:
