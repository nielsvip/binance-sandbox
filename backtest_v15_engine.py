# backtest_v15_engine — forked from backtest_v12_engine.py 2026-09-07
#!/usr/bin/env python3
"""backtest_v12_engine — LIVE-FAITHFUL scalar backtest. NO VECTORISATION.

Runs the REAL live functions. Does not reimplement them, does not approximate
them, contains no vectorised masks:

    ez_manage.process_position()
    ez_manage.calculate_final_order_quantity()
    ez_manage.check_entry_vetting() / check_entry_alignment()
    tradier_manage equivalents for stocks

Per BACKTEST_BIBLE: "NEVER present backtest from reimplemented logic. Only real
process_position(), check_entry/exit_candidates() produce valid numbers."

An earlier version of this file was built on v8_vec_sweep, a VECTORISED engine.
Wrong twice over: it reimplements live instead of calling it, and comparing two
vectorised engines cannot validate live behaviour. Kept as
backups/backtest_v12_engine_vectorised_WRONG_*.py.

ROLE IN THE PIPELINE
    v12_quick_engine      vectorised, ~0.23 s/config — sweeps millions of combos
    backtest_v12_engine   THIS, live call path, slow — verifies the few survivors

Never run this over a search space; only over candidates v12 has narrowed.

KNOWN DEAD WEIGHT: 1,981 lines of generated boilerplate of the form
    if bool(getattr(cfg,"X",False)) if "X".endswith("_ENABLED") else ...
a runtime string test on the switch's own name, always constant. They are inert
no-ops that inflate the apparent switch coverage (1,921 claimed vs 266 real via
the live call path). They are NOT stripped here — they sit inside conditional
blocks and removing them breaks indentation. Remove them properly with an AST
pass, not a line filter.
"""
...
def _curated_override_subset(overrides: dict) -> dict:
    """Hard-stop non-curated research/config operators from backtest wiring."""
    if os.environ.get("LIFECYCLE_EXACT_RECIPE_VERIFY") == "1":
        # Backtest-only parity audit: replay the exact already-live per_sym
        # baseline/candidate instead of silently dropping non-curated keys.
        # This route is deliberately unavailable unless the real engine is
        # forced and the explicit recipe lives under the lifecycle report tree.
        if os.environ.get("V8_FORCE_REAL") != "1":
            raise RuntimeError("exact lifecycle recipe requires V8_FORCE_REAL=1")
        override_file = Path(os.environ.get("V8_OVERRIDE_FILE", "")).resolve()
        lifecycle_root = (BASE_PATH / "data" / "reports" / "lifecycle_pilot").resolve()
        try:
    ...
    try:
        ...
        raise RuntimeError(f"curated ALL_PATHS allowlist unavailable: {exc}") from exc
    return {str(k): v for k, v in (overrides or {}).items() if str(k) in allowed}
    ...
    if os.environ.get("V8_FORCE_REAL", "0") == "1":
        ...
        try:
            ...
            if _auto_candidate and _auto_candidate.exists():
                ...
                if len(_auto_data) > 0:
                    # FIX 2026-08-21 WIRING: vector must read ALL per_sym switches (2032), not just sample 844/2032. Union all overrides for parity with live (live reads 98.6%).
                    _union_overrides = {}
                    for _k, _v in _auto_data.items():
                        if isinstance(_v, dict) and isinstance(_v.get("overrides"), dict):
                            _union_overrides.update(_v.get("overrides", {}))
                    ...
                    if isinstance(_sample, dict) and isinstance(_sample.get("overrides"), dict) and len(_sample.get("overrides", {})) > 0:
                        # Use union for wiring parity, not just sample
                        _overrides = _union_overrides
                        _override_source = f"AUTO_VECTOR:{_auto_candidate}:union_{len(_union_overrides)}_from_{len(_auto_data)}_syms"
...
# Canonical sweep names and the names consumed by the scalar Tradier live
# implementation are not always identical.  Keep this list in the V12 adapter
# (rather than relying on import-time config defaults) so an explicit override
# has one deterministic route through backtest_v12_engine.py.  These are name
# aliases only; they never enable a parent gate or manufacture an outcome.
_V12_TRADIER_LIVE_ALIAS_PAIRS = (
...
_V12_TRADIER_LIVE_ALIAS_EQUIVALENTS = {
    name: frozenset(pair)
    for pair in _V12_TRADIER_LIVE_ALIAS_PAIRS
    for name in pair
}


def _v12_expand_explicit_tradier_aliases(overrides: dict) -> dict:
    """Mirror an explicitly supplied alias without overriding an explicit twin."""
    expanded = dict(overrides)
    for canonical, live_name in _V12_TRADIER_LIVE_ALIAS_PAIRS:
        if canonical in overrides and live_name not in overrides:
            expanded[live_name] = overrides[canonical]
        elif live_name in overrides and canonical not in overrides:
            expanded[canonical] = overrides[live_name]
    return expanded


def _v8_probe_config_read(name: str) -> None:
    equivalents = _V12_TRADIER_LIVE_ALIAS_EQUIVALENTS.get(
        _V8_PATH_PROBE_PARAM, frozenset((_V8_PATH_PROBE_PARAM,))
    )
    if _V8_PATH_PROBE_PARAM and name in equivalents:
...
def _v8_write_path_probe_telemetry() -> None:
    ...
    try:
        ...
        read_count = sum(int(_V8_PATH_PROBE_READS.get(name, 0)) for name in equivalents)
        temporary.write_text(json.dumps({
            "schema": "direct-v8-config-read-telemetry-v1",
            "param": _V8_PATH_PROBE_PARAM,
            "equivalent_names": sorted(equivalents),
...
# Live config.MTF_ARMED_ENTRY_ENABLED  # 2026-08-23 TANDEM: bottom/top entries bypass — every path enabled filtered correctly in right TF defaults True (active protection): every
...
async def run_simulation(mode, account_key, start_date, capital, stores, resolution):
    ...
    # Enforce in backtest before any sizing: every MAX_* >= START*10 and BAND_ARROW_MULT >=10
    # Skip unlevered audit contract — it intentionally sets START==capital for $10k normalization
    if _crypto_contract != "unlevered":
        try:
            ...
            # mirror to tradier config (when present) — tradier path reads its own module
            try:
                import config_tradier as _ct
...
def main():
    ...
    if os.environ.get("V8_FORCE_REAL", "0") == "1":
        ...
        try:
            ...
            _ov = _curated_override_subset(_ov)
...
async def run_simulation_tradier(account_key, start_date, capital, stores, resolution):
    ...
    if _t_override_file and Path(_t_override_file).exists():
        ...
        _t_overrides = _v12_expand_explicit_tradier_aliases(_t_overrides)
        _t_overrides = _curated_override_subset(_t_overrides)
    ...
    for k, v in (overrides or {}).items():
        ...
        if mode == "tradier" and k.startswith("TRADIER_"):
            ...
            if clean not in keys:
                keys.append(clean)
        elif mode == "tradier" and not k.startswith("TRADIER_"):
            # also mirror non-prefixed to prefixed for tradier engine reads that expect TRADIER_*
            keys.append("TRADIER_" + k)
        for kk in keys:
            # snapshot once per distinct key
            if kk not in snap:
                # try crypto config then tradier config
                cur = None
                try:
                    cur = getattr(_cfg, kk, None)
                except Exception:
                    cur = None
                try:
                    if cur is None:
                        cur = getattr(_ct.TradierConfig, kk, None)
                except Exception:
                    pass
                snap[kk] = cur
            # apply to crypto config module / class / dataclass / instances
            try:
                setattr(_cfg, kk, v)
            except Exception:
                pass
            try:
                setattr(_cfg.Config, kk, v)
            except Exception:
                pass
            try:
                if hasattr(_cfg.Config, '__dataclass_fields__') and kk in _cfg.Config.__dataclass_fields__:
                    _cfg.Config.__dataclass_fields__[kk].default = v
            except Exception:
                pass
            try:
                for _inst in list(getattr(_cfg.Config, '_INSTANCES', [])):
                    setattr(_inst, kk, v)
            except Exception:
                pass
            # also apply to tradier config when mode tradier
            if mode == "tradier":
                try:
                    setattr(_ct, kk, v)
                except Exception:
                    pass
                try:
                    setattr(_ct.TradierConfig, kk, v)
                except Exception:
                    pass
                try:
                    if hasattr(_ct.TradierConfig, '__dataclass_fields__') and kk in _ct.TradierConfig.__dataclass_fields__:
                        _ct.TradierConfig.__dataclass_fields__[kk].default = v
                except Exception:
                    pass
                try:
                    import tradier_manage as _tm
                    if hasattr(_tm, 'config'):
                        setattr(_tm.config, kk, v)
                except Exception:
                    pass
                try:
                    for _inst in list(getattr(_ct.TradierConfig, '_INSTANCES', [])):
                        setattr(_inst, kk, v)
                except Exception:
                    pass
    return snap
...
def _run_one_restore_overrides(snap: dict, mode: str):
    if not snap:
        return
    import config as _cfg
    import config_tradier as _ct
    for kk, orig in snap.items():
        try:
            if orig is None:
                try:
                    delattr(_cfg, kk)
                except Exception:
                    setattr(_cfg, kk, orig)
            else:
                setattr(_cfg, kk, orig)
        except Exception:
            pass
        try:
            setattr(_cfg.Config, kk, orig)
        except Exception:
            pass
        try:
            if hasattr(_cfg.Config, '__dataclass_fields__') and kk in _cfg.Config.__dataclass_fields__:
                _cfg.Config.__dataclass_fields__[kk].default = orig
        except Exception:
            pass
        for _inst in list(getattr(_cfg.Config, '_INSTANCES', [])):
            try:
                setattr(_inst, kk, orig)
            except Exception:
                pass
        if mode == "tradier":
            try:
                setattr(_ct, kk, orig)
                setattr(_ct.TradierConfig, kk, orig)
                if hasattr(_ct.TradierConfig, '__dataclass_fields__') and kk in _ct.TradierConfig.__dataclass_fields__:
                    _ct.TradierConfig.__dataclass_fields__[kk].default = orig
            except Exception:
                pass
            try:
                import tradier_manage as _tm
                if hasattr(_tm, 'config'):
                    setattr(_tm.config, kk, orig)
            except Exception:
                pass
            for _inst in list(getattr(_ct.TradierConfig, '_INSTANCES', [])):
                try:
...
def _enable_npz_parity() -> None:
    ...
    ez_manage._NPZ_PARITY_MODE / _NPZ_STRICT_PARITY make the live decision
    functions read a frozen NPZ instead of real-time indicators, returning a
    hardcoded default when a key is absent. That is exactly what a backtest wants
    (apples-to-apples against v12_quick_engine over the same frozen window) and
    exactly what LIVE must never do. They shipped defaulting True, which put live
    trb/trc on weeks-old data. Now they default False and are set here.
    """
    import os

    os.environ["V12_PARITY_MODE"] = "1"
    os.environ["V12_PARITY_STRICT"] = "1"
    try:
        import ez_manage as _ez

        _ez._NPZ_PARITY_MODE = True
        _ez._NPZ_STRICT_PARITY = True
    except Exception:
        pass
...
def run_one(symside, overrides=None, window_days=365, offset_days=0, targets=None):
    ...
    executed = []
    try:
        stores, resolution = load_stores(mode, symbols=[sym], start_date=start_date)
        if not stores or sym not in stores:
            # no NPZ: invalid run, matches evaluate_v12 semantics
            return {"symside": symside, "valid": False, "invalid_reason": "no npz", "gain_per_mo": 0.0, "trades": 0, "gain_pct": 0.0, "pool_sharpe": 0.0, "max_dd_pct": 0.0, "tim_pct": 0.0, "score": float("-inf")}
        # ensure live path guard passes: import live modules so _assert_live_path sees them
        if mode == "crypto":
            import ez_manage as _ezm  # noqa
        ...
        try:
            if mode == "crypto":
                ...
                executed = _asyncio.run(run_simulation(mode, "ang", start_date, 10000.0, stores, resolution))
            else:
                executed = _asyncio.run(run_simulation_tradier("trb", start_date, 10000.0, stores, resolution))
        finally:
            if _prev_sweep is None:
    ...
    try:
        ...
        import numpy as _np
        # build trade_returns from pnl_pct of closes
        trs = []
        events = []
        ...
        for ev in (executed or []):
            ...
            try:
                if hasattr(ts_raw, "timestamp"):
                    ...
                    ts = float(_dt.datetime.fromisoformat(ts_raw.replace("Z","+00:00")).timestamp())
                else:
                    ts = 0.0
            except Exception:
                ts = 0.0
            qty = float(ev.get("quantity", ev.get("qty", 0)) or 0)
            px = float(ev.get("price", 0) or 0)
            val = abs(qty * px)
            etype = str(ev.get("type", ev.get("action", "")) or "").upper()
            # normalize: eta events are all "eta" with action field holding OPEN/CLOSE etc
            act = str(ev.get("action", "") or "").upper()
            if etype == "ETA":
                if act in ("OPEN","QUICK_OPEN","AUGMENT","QUICK_AUGMENT","REENTRY","HEDGE_OPEN"):
                    etype = "OPEN"
                elif act in ("CLOSE","REDUCE","QUICK_CLOSE","FULL_CLOSE","HEDGE_CLOSE","MTM_FINAL_BAR_NOLIES_RULE2") or "CLOSE" in act or "REDUCE" in act:
                    etype = "CLOSE"
                else:
                    etype = act or "OPEN"
            ...
            class _Ev:
                pass
            e = _Ev()
            e.ts = ts; e.type = etype; e.qty = qty; e.value = val; e.pnl_pct = pnl_f
            events.append(e)
        # time window from NPZ timestamps (fallback to start_date)
        try:
            npz_store = stores.get(sym) if isinstance(stores, dict) else None
            if npz_store is not None and hasattr(npz_store, "timestamps"):
                _ts_arr = _np.asarray(npz_store.timestamps) if hasattr(npz_store, "timestamps") else None
            else:
                # stores[sym] is IndicatorStore; try to get timestamps via npz file fallback
                _ts_arr = None
                try:
                    from tools.infra_paths import get_npz_dir as _gnd
                    _p = _gnd() / f"{sym}.npz"
                    if _p.exists():
                        _d = dict(_np.load(str(_p), allow_pickle=True))
                        _ts_arr = _d.get("timestamps", _d.get("timestamp_5m", _d.get("timestamp_3m")))
                except Exception:
                    _ts_arr = None
            if _ts_arr is not None and len(_ts_arr) >= 2:
                t_start = float(_ts_arr[0]); t_end = float(_ts_arr[-1])
            else:
                t_start = float(_dt.datetime.strptime(start_date, "%Y-%m-%d").replace(tzinfo=_dt.timezone.utc).timestamp())
                t_end = float(_dt.datetime.now(_dt.timezone.utc).timestamp())
        except Exception:
            t_start = 0.0; t_end = float(window_days*86400)
        # BH baseline from NPZ close (same as evaluate_v12._bh)
        bh_pct = None
        try:
            from tools.opt import evaluate_v12 as _EV
            # try to get npz dict for BH via V._store
            import datetime as _d2
            _npz = _EV._store(sym, mode, start_date)
            if _npz is not None:
                _c = _npz.get("close") if isinstance(_npz, dict) else None
                if _c is not None and len(_np.asarray(_c)) >= 2:
                    _a = _np.asarray(_c, dtype=float)
                    _a = _a[_np.isfinite(_a) & (_a>0)]
                    if len(_a) >= 2:
                        raw = (_a[-1]-_a[0])/_a[0]*100.0
                        bh_pct = raw if side=="LONG" else max(-100.0, -raw)
        except Exception:
            bh_pct = None
...
def parity(symside, overrides=None, tol=0.20, window_days=365, offset_days=0):
    """Compare vector vs live on the same overrides/window. tol is relative on gain/mo."""
    from tools.opt import evaluate_v12 as _EV
    ov = dict(overrides or {})
    # if a reconciliation file exists, caller already merged it; here we just compare straight
    vec = _EV.evaluate(symside, ov, window_days=window_days, offset_days=offset_days)
    live = run_one(symside, ov, window_days=window_days, offset_days=offset_days)
    # when both invalid, parity is moot -> report blocked
    if not vec.get("valid") and not live.get("valid"):
        return {"ok": False, "checked": True, "rel_diff": None, "vec_gain_per_mo": vec.get("gain_per_mo"), "twin_gain_per_mo": live.get("gain_per_mo"), "reason": f"both invalid vec:{vec.get('invalid_reason')} live:{live.get('invalid_reason')}"}
    vg = float(vec.get("gain_per_mo") or 0.0); lg = float(live.get("gain_per_mo") or 0.0)
    denom = max(1e-9, abs(vg))
    rel = abs(lg - vg) / denom
    ok = rel <= float(tol)
