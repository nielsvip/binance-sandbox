...
def evaluate_uve_signals(npz: Dict[str, np.ndarray], is_long: bool, mode: str) -> Dict[str, np.ndarray]:
    n = len(npz.get("timestamp_15m", npz.get("timestamp_5m", npz.get("timestamp_3m"))))
    ...
    anti_parabolic = (stoch_k_15m < 90.0) & (stoch_k_1h < 90.0)
...
def simulate_uve(npz: Dict[str, np.ndarray], is_long: bool, mode: str) -> Dict[str, Any]:
    ...
    for i in range(n):
        ...
        if not position_active:
            if entry_allowed[i]:
                ...
                events.append({"idx": i, "type": "OPEN", "price": mark, "qty": qty, "reason": "UVE_WT_ENTRY"})
        else:
            ...
            if gain_pct >= 1.5 and not ppl_fired:
                ppl_fired, qty = True, qty * 0.5
                events.append({"idx": i, "type": "REDUCE", "price": mark, "qty": qty, "reason": "UVE_PPL_STEP1"})
            trail_stop = (peak_price - 2.0 * current_atr) if is_long else (peak_price + 2.0 * current_atr)
            ...
            if should_exit:
                ...
                events.append({"idx": i, "type": "CLOSE", "price": mark, "qty": qty, "reason": "UVE_ATR_TRAIL" if not ppl_fired else "UVE_PPL_EXIT"})
                ...
                if augment_gain >= 3.0:
                    ...
                    events.append({"idx": i, "type": "AUGMENT", "price": mark, "qty": 0.5, "reason": "UVE_AUGMENT"})
    if position_active:
        mark = float(close[-1])
        gain_pct = (mark - entry_price) / entry_price * 100.0 if is_long else (entry_price - mark) / entry_price * 100.0
        returns.append(gain_pct)
        events.append({"idx": n - 1, "type": "CLOSE", "price": mark, "qty": qty, "reason": "UVE_MTM_FINAL"})
