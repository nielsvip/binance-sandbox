...
class QuickConfig:
    ...
    # === 2026-04-30 DEAD-SWITCH wiring — companions for the 9 baseline switches ===
    # SATOSHIT_ENTRY_TRADIER: explicit gate to enable the SATOSHIT 5-vote entry path
    # (separate from SATOSHIT_ENABLED which already drives the EXIT path). Default OFF.
    SATOSHIT_ENTRY_TRADIER_ENABLED: bool = False
    # BB_SQUEEZE entry signal — multi-TF squeeze_on alignment + WT bias.
    BB_SQUEEZE_ENTRY_ENABLED: bool = False
    # REGIME_GATE_ENABLED governs the REGIME_ENTER_TRENDING_THRESHOLD ADX gate.
    # (Already defined upstream as REGIME_GATE_ENABLED — kept for reference.)
...
def compute_entry_signals(npz, n, is_long, cfg):
    ...
    # ═══ 2026-04-30 DEAD-SWITCH WIRING — 9 baseline switches that were definition-only ═══
    # MOM3_ENTRY_ENABLED: live ez_manage:17936 — mean-reversion add to score when 3-bar move
    # is strongly opposite. LONG fires when (close-close[i-3])/close[i-3] < MOM3_LONG_THRESHOLD.
    # NPZ field: close_{LTF} (5m for tradier, 3m for crypto). Treated as ADDITIVE entry signal.
    mom3_sig = np.zeros(n, dtype=bool)
    if bool(getattr(cfg, 'MOM3_ENTRY_ENABLED', False)):
        try:
            _close_ltf = _safe(npz, f'close_{_ltf}', n, 0.0)
            if _close_ltf.sum() > 0:
                _close_ltf_3 = np.roll(_close_ltf, 3); _close_ltf_3[:3] = _close_ltf[:3]
                _safe_c3 = np.where(_close_ltf_3 > 0, _close_ltf_3, 1.0)
                _mom3_pct = (_close_ltf - _close_ltf_3) / _safe_c3 * 100.0
                _mom3_long_thr = float(getattr(cfg, 'MOM3_LONG_THRESHOLD', -1.0))
                _mom3_short_thr = float(getattr(cfg, 'MOM3_SHORT_THRESHOLD', 1.0))
                if is_long:
                    mom3_sig = (_close_ltf_3 > 0) & (_mom3_pct < _mom3_long_thr)
                else:
                    mom3_sig = (_close_ltf_3 > 0) & (_mom3_pct > _mom3_short_thr)
        except Exception:
            pass
    # SATOSHIT entry tradier (live ez_satoshit.py:115) — 5-vote system on
    # rsi_15m / bb_pct_b_1h / ha_15m / stoch_k_15m / mfi_15m + HTF MFI_D / rvol_1h floors.
    # Pass when votes >= SATOSHIT_MIN_VOTES_TRADIER. ADDITIVE entry path (alternative path).
    sat_trad_sig = np.zeros(n, dtype=bool)
    if (bool(getattr(cfg, 'SATOSHIT_ENTRY_TRADIER_ENABLED', False))
            and getattr(cfg, 'MODE', 'crypto') == 'tradier'):
        try:
            _rsi_15m = _safe(npz, 'rsi_15m', n, 50)
            _k_15m_sat = _safe(npz, 'stoch_k_15m', n, 50)
            _mfi_15m_sat = _safe(npz, 'mfi_15m', n, 50)
            _bb_pctb_1h_sat = _safe(npz, 'bb_pct_b_1h', n, 0.5)
            _ha_15m_sat = _ha_int(npz, 'ha_15m', n)
            _mfi_D_sat = _safe(npz, 'mfi_D', n, 50)
            _rvol_1h_sat = _safe(npz, 'relative_volume_1h', n, 1.0)
            if is_long:
                v_rsi = (_rsi_15m < float(getattr(cfg, 'SATOSHIT_LONG_RSI_MAX_TRADIER', 50.0))).astype(np.int8)
                v_bb = (_bb_pctb_1h_sat < float(getattr(cfg, 'SATOSHIT_LONG_BB_PCTB_MAX', 0.50))).astype(np.int8)
                v_ha = (_ha_15m_sat < int(getattr(cfg, 'SATOSHIT_LONG_HA_STREAK_MAX', 1))).astype(np.int8)
                v_k = (_k_15m_sat < float(getattr(cfg, 'SATOSHIT_LONG_STOCH_K_MAX_TRADIER', 60.0))).astype(np.int8)
                v_mfi = (_mfi_15m_sat < float(getattr(cfg, 'SATOSHIT_LONG_MFI_MAX_TRADIER', 60.0))).astype(np.int8)
            else:
                v_rsi = (_rsi_15m > float(getattr(cfg, 'SATOSHIT_SHORT_RSI_MIN_TRADIER', 55.0))).astype(np.int8)
                v_bb = (_bb_pctb_1h_sat > float(getattr(cfg, 'SATOSHIT_SHORT_BB_PCTB_MIN', 0.55))).astype(np.int8)
                v_ha = (_ha_15m_sat > int(getattr(cfg, 'SATOSHIT_SHORT_HA_STREAK_MIN', 0))).astype(np.int8)
                v_k = (_k_15m_sat > float(getattr(cfg, 'SATOSHIT_SHORT_STOCH_K_MIN_TRADIER', 50.0))).astype(np.int8)
                v_mfi = (_mfi_15m_sat > float(getattr(cfg, 'SATOSHIT_SHORT_MFI_MIN_TRADIER', 50.0))).astype(np.int8)
            _votes_sat = v_rsi + v_bb + v_ha + v_k + v_mfi
            _min_votes_sat = int(getattr(cfg, 'SATOSHIT_MIN_VOTES_TRADIER', 3))
            _htf_mfi_min = float(getattr(cfg, 'SATOSHIT_HTF_MFI_D_MIN_TRADIER', 30.0))
            _htf_rvol_min = float(getattr(cfg, 'SATOSHIT_HTF_RVOL_1H_MIN_TRADIER', 0.3))
            sat_trad_sig = (_votes_sat >= _min_votes_sat) & (_mfi_D_sat >= _htf_mfi_min) & (_rvol_1h_sat >= _htf_rvol_min)
        except Exception:
            pass
    # TRADIER_FH_MOMENTUM_ENABLED — auto-fires entries during first-hour-after-open if move
    # crosses MIN_MOVE_PCT, gated by MFI_D and DC_position_D confirmation.
    # NPZ: timestamps, open_D, close_{LTF}, mfi_D, dc_position_D. Acts as ADDITIVE entry path.
    fh_mom_sig = np.zeros(n, dtype=bool)
    if (bool(getattr(cfg, 'TRADIER_FH_MOMENTUM_ENABLED', False))
            and getattr(cfg, 'MODE', 'crypto') == 'tradier'):
        try:
            ts_fh = npz.get('timestamps', None)
            if ts_fh is not None and len(ts_fh) > 0:
                ts_fh = np.asarray(ts_fh, dtype=np.int64)[:n]
                if len(ts_fh) < n:
                    pad_fh = np.full(n - len(ts_fh), ts_fh[-1] if len(ts_fh) else 0, dtype=np.int64)
                    ts_fh = np.concatenate([ts_fh, pad_fh])
                _sec_of_day_fh = ts_fh % 86400
                _fh_win_min = int(getattr(cfg, 'TRADIER_FH_MOMENTUM_WINDOW_MINUTES', 60))
                _fh_window = (_sec_of_day_fh >= 48600) & (_sec_of_day_fh < 48600 + _fh_win_min * 60)
                _open_D_fh = _safe(npz, 'open_D', n, 0.0)
                _safe_open_fh = np.where(_open_D_fh > 0, _open_D_fh, 1.0)
                _fh_move_pct = (close - _open_D_fh) / _safe_open_fh * 100.0
                _fh_min = float(getattr(cfg, 'TRADIER_FH_MOMENTUM_MIN_MOVE_PCT', 0.5))
                _fh_dir_ok = (_fh_move_pct >= _fh_min) if is_long else (_fh_move_pct <= -_fh_min)
                _fh_mfi_ok = np.ones(n, dtype=bool)
                if bool(getattr(cfg, 'TRADIER_FH_MOMENTUM_MFI_CONFIRM', True)):
                    _fh_mfi_D = _safe(npz, 'mfi_D', n, 50.0)
                    if is_long:
                        _fh_mfi_ok = _fh_mfi_D >= 50.0
                    else:
                        _fh_mfi_ok = _fh_mfi_D <= 50.0
                _fh_dc_ok = np.ones(n, dtype=bool)
                if bool(getattr(cfg, 'TRADIER_FH_MOMENTUM_DC_CONFIRM', True)):
                    _fh_dc_D = _safe(npz, 'dc_position_D', n, 0.5)
                    _fh_dc_max = float(getattr(cfg, 'TRADIER_FH_MOMENTUM_DC_MAX_LONG', 0.33))
                    if is_long:
                        _fh_dc_ok = _fh_dc_D <= _fh_dc_max
                    else:
                        _fh_dc_ok = _fh_dc_D >= (1.0 - _fh_dc_max)
                fh_mom_sig = _fh_window & (_open_D_fh > 0) & _fh_dir_ok & _fh_mfi_ok & _fh_dc_ok
        except Exception:
            pass
    # REGIME_ENTER_TRENDING_THRESHOLD — gate entries to "trending" regime via ADX_1h ≥ threshold.
    # Live ez_manage:189 ADX_REGIME_FILTER_ENABLED uses adx_{ADX_TF}. Use threshold from baseline.
    # NPZ: adx_1h. Pass-through when adx data missing.
    regime_ok = np.ones(n, dtype=bool)
    if bool(getattr(cfg, 'REGIME_GATE_ENABLED', False)):
        try:
            _adx_reg = _safe(npz, 'adx_1h', n, 0.0)
            if _adx_reg.max() > 0:
                _reg_min = float(getattr(cfg, 'REGIME_ENTER_TRENDING_THRESHOLD', 30.0))
                regime_ok = _adx_reg >= _reg_min
        except Exception:
            pass
    # BB_SQUEEZE_MIN_ALIGNMENT — count number of TFs in squeeze_on; fire entry when ≥ MIN_ALIGNMENT
    # in matching direction (squeeze release with WT bias). NPZ: squeeze_on_{15m,1h,4h,D},
    # wt_bullish_{tf}. Acts as ADDITIVE entry path (OR onto base_sig).
    bb_squeeze_sig = np.zeros(n, dtype=bool)
    if bool(getattr(cfg, 'BB_SQUEEZE_ENTRY_ENABLED', False)):
        try:
            _sq_15m = _safe(npz, 'squeeze_on_15m', n, 0).astype(np.int8)
            _sq_1h = _safe(npz, 'squeeze_on_1h', n, 0).astype(np.int8)
            _sq_4h = _safe(npz, 'squeeze_on_4h', n, 0).astype(np.int8)
            _sq_D = _safe(npz, 'squeeze_on_D', n, 0).astype(np.int8)
            _sq_W = _safe(npz, 'squeeze_on_W', n, 0).astype(np.int8)
            _sq_M = _safe(npz, 'squeeze_on_M', n, 0).astype(np.int8)
            # MIN_ALIGNMENT is interpreted as fraction*10 of TFs (baseline 5 → ≥3 of 6 TFs).
            _sq_count = _sq_15m + _sq_1h + _sq_4h + _sq_D + _sq_W + _sq_M
            _sq_min = int(getattr(cfg, 'BB_SQUEEZE_MIN_ALIGNMENT', 10))
            _sq_need = max(1, min(6, int(_sq_min // 2)))
            _wt_bull_1h_sq = _safeb(npz, 'wt_bullish_1h', n)
            _wt_bull_4h_sq = _safeb(npz, 'wt_bullish_4h', n)
            if is_long:
                bb_squeeze_sig = (_sq_count >= _sq_need) & _wt_bull_1h_sq & _wt_bull_4h_sq
            else:
                bb_squeeze_sig = (_sq_count >= _sq_need) & (~_wt_bull_1h_sq) & (~_wt_bull_4h_sq)
        except Exception:
            pass
    # RSI_MOMENTUM_MODE — invert RSI gating from mean-reversion (low RSI = LONG entry) to
    # momentum (rising RSI = LONG entry). Requires RSI_1h to be RISING for LONG, FALLING for SHORT.
    # NPZ: rsi_1h. Acts as a GATE on entries (filter, not additive).
    rsi_mom_ok = np.ones(n, dtype=bool)
    if bool(getattr(cfg, 'RSI_MOMENTUM_MODE', False)):
        try:
            _rsi_1h_mom = _safe(npz, 'rsi_1h', n, 50.0)
            _rsi_1h_prev_mom = np.roll(_rsi_1h_mom, _bph_1h)
            _rsi_1h_prev_mom[:_bph_1h] = _rsi_1h_mom[:_bph_1h]
            if is_long:
                rsi_mom_ok = (_rsi_1h_mom > _rsi_1h_prev_mom) & (_rsi_1h_mom >= 50.0)
            else:
                rsi_mom_ok = (_rsi_1h_mom < _rsi_1h_prev_mom) & (_rsi_1h_mom <= 50.0)
        except Exception:
            pass
    # BASIS_CONDITION — price relative to dc_basis_1h (Donchian basis). LONG entries require
    # close > dc_basis_1h (above basis = trend up); SHORT requires close < dc_basis_1h.
    # NPZ: dc_basis_1h. Acts as a GATE.
    basis_ok = np.ones(n, dtype=bool)
    if bool(getattr(cfg, 'BASIS_CONDITION', False)):
        try:
            _dcb_1h = _safe(npz, 'dc_basis_1h', n, 0.0)
            if _dcb_1h.max() > 0:
                if is_long:
                    basis_ok = close > _dcb_1h
                else:
                    basis_ok = close < _dcb_1h
        except Exception:
            pass
    base_sig = raw & kltf_ok & ct_vel_ok & ct_dc_ok & htf_ok & mfi_gate & vwap_ok & extra_ok & mtf_vel_ok & cross_fresh_ok & le_ok & minervini_ok & clenow_ok & proximity_top_ok & regime_ok & rsi_mom_ok & basis_ok
    # ADDITIVE signals from dead-switch wiring — OR onto base_sig (alternative entry paths).
    if bool(getattr(cfg, 'MOM3_ENTRY_ENABLED', False)):
        base_sig = base_sig | mom3_sig
    if (bool(getattr(cfg, 'SATOSHIT_ENTRY_TRADIER_ENABLED', False))
            and getattr(cfg, 'MODE', 'crypto') == 'tradier'):
        base_sig = base_sig | sat_trad_sig
    if (bool(getattr(cfg, 'TRADIER_FH_MOMENTUM_ENABLED', False))
            and getattr(cfg, 'MODE', 'crypto') == 'tradier'):
        base_sig = base_sig | fh_mom_sig
    if bool(getattr(cfg, 'BB_SQUEEZE_ENTRY_ENABLED', False)):
        base_sig = base_sig | bb_squeeze_sig
...
def compute_exit_signals(npz, n, is_long, cfg):
    ...
    if getattr(cfg, 'SATOSHIT_EXIT_ENABLED', False):
        ...
        if votes_needed >= 3:
            ...
            extra_exit = extra_exit | (rsi_hit | stoch_hit)
    # BB_RECOVERY_EXIT_ENABLED_TRADIER — close LONG that entered above bb_upper_1h once price
    # recovers back near entry, with 3m oscillator turning. Live tradier_manage:8395 — bypasses
    # NOLOSS-min when recovery condition met. In v8_quick (no NOLOSS gate) this acts as an
    # accept-loss/scratch exit after extreme over-extended entry. Tradier-only.
    if (bool(getattr(cfg, 'BB_RECOVERY_EXIT_ENABLED_TRADIER', False))
            and getattr(cfg, 'MODE', 'crypto') == 'tradier'):
        try:
            _bbu_1h_br = _safe(npz, 'bb_upper_1h', n, 0.0)
            _bbl_1h_br = _safe(npz, 'bb_lower_1h', n, 0.0)
            _k_3m_br = _safe(npz, f'stoch_k_{_ltf}', n, 50)
            _k_3m_br_prev = np.roll(_k_3m_br, 1); _k_3m_br_prev[0] = _k_3m_br[0]
            _ha_ltf_br = _ha_int(npz, f'ha_{_ltf}', n)
            # Approximate "entry above band" by current bb-extreme proximity AND opposite-momentum oscillator.
            # LONG: close near/below bb_upper_1h AND k turning down (oscillator reversing).
            # SHORT: close near/above bb_lower_1h AND k turning up.
            if is_long:
                _br_proxim = (_bbu_1h_br > 0) & (close < _bbu_1h_br * 1.005) & (close > _bbu_1h_br * 0.99)
                _br_reverse = (_k_3m_br < _k_3m_br_prev) | (_ha_ltf_br < 0)
                extra_exit = extra_exit | (_br_proxim & _br_reverse)
            else:
                _br_proxim = (_bbl_1h_br > 0) & (close > _bbl_1h_br * 0.995) & (close < _bbl_1h_br * 1.01)
                _br_reverse = (_k_3m_br > _k_3m_br_prev) | (_ha_ltf_br > 0)
                extra_exit = extra_exit | (_br_proxim & _br_reverse)
        except Exception:
            pass
