...
def generate_sym_chart(sym: str, npz: dict, trades: List[Dict], m: Dict,
    ...
    plot_ts = ts_arr[mask]; plot_close = close_arr[mask]
    if len(plot_ts) == 0:
        return None
    trades_win = [t for t in trades if t.get("exit_ts", 0) >= cutoff_ts] or trades
    pnl = [float(t.get("pnl_pct", 0)) for t in trades_win]
    cum_pnl = []; running = 0.0
    for p in pnl:
        running += p; cum_pnl.append(running)
    dt_price = [datetime.fromtimestamp(int(ts), tz=timezone.utc) for ts in plot_ts if ts > 0]
    dt_entry = [datetime.fromtimestamp(int(t["entry_ts"]), tz=timezone.utc) for t in trades_win if t.get("entry_ts", 0) > 0]
    ep_list = [float(t.get("entry_price", 0)) for t in trades_win if t.get("entry_ts", 0) > 0]
    dt_exit = [datetime.fromtimestamp(int(t["exit_ts"]), tz=timezone.utc) for t in trades_win if t.get("exit_ts", 0) > 0]
    ...
    dt_cum = [datetime.fromtimestamp(int(t.get("exit_ts", 0)), tz=timezone.utc) for t in trades_win if t.get("exit_ts", 0) > 0]
    n_win = sum(1 for p in pnl if p > 0)
    fig = plt.figure(figsize=(18, 11), facecolor="#0d1117"); fig.patch.set_facecolor("#0d1117")
    gs = gridspec.GridSpec(3, 1, height_ratios=[3, 1.2, 1], hspace=0.06, figure=fig)
    ax_p = fig.add_subplot(gs[0]); ax_p.set_facecolor("#0d1117")
    if len(dt_price) == len(plot_close): ax_p.plot(dt_price, plot_close, color="#c9d1d9", linewidth=0.8, zorder=2)
    if dt_entry and len(dt_entry) == len(ep_list):
        ...
        if longs: ax_p.scatter([x[0] for x in longs], [x[1] for x in longs], marker="^", s=55, color="#3fb950", zorder=5, label=f"LONG ({len(longs)})")
        if shorts: ax_p.scatter([x[0] for x in shorts], [x[1] for x in shorts], marker="v", s=55, color="#f0883e", zorder=5, label=f"SHORT ({len(shorts)})")
    if dt_exit and len(dt_exit) == len(xp_list): ax_p.scatter(dt_exit, xp_list, marker="x", s=35, color="#f85149", zorder=5, linewidths=1.3, label=f"Exit ({len(dt_exit)})")
    ax_p.set_title(f"men | {sym}  pool={m['pool_sharpe']:+.4f}  wr={m['wr_pct']:.1f}%  dd={m['max_dd_pct']:.2f}%  trades={m['trades']}  wins={n_win}/{len(pnl)}  ({days}d)", color="#c9d1d9", fontsize=10, pad=6)
    ax_p.set_ylabel("Price", color="#c9d1d9", fontsize=9); ax_p.tick_params(colors="#6e7681", labelsize=7)
    ...
    ax_c = fig.add_subplot(gs[1], sharex=ax_p); ax_c.set_facecolor("#0d1117")
    if dt_cum and len(dt_cum) == len(cum_pnl): ax_c.plot(dt_cum, cum_pnl, color="#58a6ff", linewidth=1.3, zorder=3); ax_c.fill_between(dt_cum, cum_pnl, alpha=0.12, color="#58a6ff")
    ax_c.axhline(0, color="#6e7681", linewidth=0.5, linestyle="--"); ax_c.set_ylabel("Cum PnL %", color="#c9d1d9", fontsize=9)
    ...
    ax_b = fig.add_subplot(gs[2], sharex=ax_p); ax_b.set_facecolor("#0d1117")
    if dt_cum and len(dt_cum) == len(pnl): ax_b.bar(dt_cum, pnl, color=["#3fb950" if p > 0 else "#f85149" for p in pnl], width=0.4, zorder=3)
    ax_b.axhline(0, color="#6e7681", linewidth=0.5, linestyle="--"); ax_b.set_ylabel("Trade %", color="#c9d1d9", fontsize=9)
    ...
    out_path = out_dir / f"men_{sym}_{days}d.png"
    plt.savefig(out_path, dpi=150, bbox_inches="tight", facecolor=fig.get_facecolor()); plt.close(fig)
