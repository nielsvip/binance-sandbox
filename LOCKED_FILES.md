...
| `ez_positions_quick.py` | RELOCKED 2026-04-20 | **2026-04-20 NEW**: Hedge vintage gain-based close paths gated OFF by default (require explicit config flag to re-enable): HEDGE_BANDAID_OFF_ENABLED, HEDGE_DECAY_NUKE_ENABLED (×2 locations), HEDGE_RECOVERY_CLOSE_ENABLED, HEDGE_PROFIT_PROTECT_ENABLED, HEDGE_LOSS_KILL_ENABLED, HEDGE_MAX_AGE_HOURS (default changed 6→0). ONLY ACTIVE closes: HEDGE_CLOSE_WT3M1H_ABS (wt_3m + wt_1h both against, no P/L gate) + HEDGE_WT_KILL (loser WT recovering) + HEDGE_ORPHAN_HEALTH_KILL + CLEANUP_STALE_HEDGE_24H. Backup: before_hedge_vintage_gate_*. DO NOT re-enable gain-based paths without sweep proof. DO NOT remove HEDGE_CLOSE_WT3M1H_ABS. **Prior (2026-04-20 20:05)**: GAIN_EROSION bypass. **Prior (2026-04-20 20:00)**: PEAK_GIVEBACK threshold 0→0.08. DO NOT revert threshold below 0.08. DO NOT remove delta_sig, tracker sync, hedge engine, WT_GATE_BYPASS_RZ, wt_3m hedge kill, PARABOLIC_EXHAUSTION_EXIT, _hedge_entry_is_valid, GAIN_EROSION, STRUCTURAL_RANGE_SHIFT_EXIT cascade, or the PREEMPTIVE_BREAKEVEN toggle. | user |
