...
int vec_same_entry_partial_scan(
    ...
    const uint8_t *fast_event, const int64_t *fast_source,
    const double *fast_ref,
    int slow_mode, const uint8_t *slow_event,
    const int64_t *slow_source, const double *slow_raw_stop,
    const double *slow_ref, const uint8_t *regime_aligned,
    double first_clip, double second_clip, int regime_switch,
    double commission, double slippage,
    SameEntryPartialMetrics *out
) {
    if (!out || n<=0 || left<0 || right>n || right-left<100 ||
        (side!=1 && side!=-1) || (semantics!=0 && semantics!=1) ||
        (slow_mode!=1 && slow_mode!=2) ||
        !(first_clip>0.0) || first_clip+second_clip>1.0+1e-12 ||
        second_clip<0.0) return 1;
    ...
    double last_exit=NAN, reclaim_level=NAN;
    double trail_stop=NAN;
    int exit_fill_row=-1, gap_seen=0, clip_stage=0;
    ...
    for (int i=left; i<right; ++i) {
        ...
        if (pending) {
            ...
            if (ts[i]>ts[pending_signal] &&
                ...
                if (pending==2) {
                    ...
                    last_exit=px; reclaim_level=level;
                    exit_fill_row=i; gap_seen=0; clip_stage=0;
                }
        ...
        for (int j=0; !pending && j<obligation_count;) {
            ...
            if (actual>0.0) {
                ...
                avg_entry=(!isfinite(avg_entry)||qty<=0.0)
                    ? px : (avg_entry*qty+px*add_qty)/new_qty;
                cash -= side*actual+commission*actual;
                qty=new_qty; out->entry_fills++; out->reclaim_reentries++;
                peak_post=dmax(peak_post,qty*px);
                o->remaining-=actual;
            }
            if (o->remaining<=1e-7) {
        ...
        if (qty>1e-12 && slow_trigger) {
            ...
            pending=3; pending_signal=i; pending_ref=slow_ref[i];
            trail_stop=NAN;
            continue;
        }
        if (qty>1e-12 && fast_event[i] && clip_stage<2) {
            if (fast_source[i]>ts[i]) out->future_htf_count++;
            double fraction=clip_stage==0 ? first_clip : second_clip;
            if (clip_stage==1 && second_clip<=1e-12) clip_stage=2;
            else if (regime_switch && regime_aligned[i]) {
                out->regime_vetoed_partial_signals++;
            } else {
                out->partial_signals++;
                pending=2; pending_signal=i; pending_ref=fast_ref[i];
                pending_fraction=fraction; clip_stage++;
                continue;
            }
        }
