#!/usr/bin/env python3
"""Memory-bounded top-10 LONG/bottom-10 SHORT peak-giveback cohort."""
...
def _one(row: dict[str, Any], npz_dir: Path, output: Path) -> dict[str, Any]:
    ...
    proc = subprocess.run(
        [
            sys.executable,
            str(ROOT / "tools" / "vec_same_entry_peak_giveback_adapter.py"),
            "--artifact",
            str(artifact),
            "--npz-dir",
            str(npz_dir),
    ...
    return {
        ...
        "validation_peak_signals": validation["peak_giveback_signals"],
        "validation_actual_exits": validation["technical_exit_fills"],
        "validation_partial_exits": validation["partial_exit_fills"],
        "validation_full_exits": validation["full_exit_fills"],
        "validation_partial_pnl_net_usd": validation[
...
def main() -> int:
            ...
            json.dumps(
                {
                    ...
                    "signals": result.get("validation_peak_signals"),
                    "partial": result.get("validation_partial_exits"),
                    "full": result.get("validation_full_exits"),
                },
                sort_keys=True,
            ),
            flush=True,
    ...
    payload = {
        "tier": "VEC_RESEARCH_SAME_ENTRY_PEAK_GIVEBACK_COHORT",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "matrix_written": False,
        "promotion_allowed": False,
        "candidate_count_per_key": 60,
