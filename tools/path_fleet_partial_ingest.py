#!/usr/bin/env python3
"""Ingest the frozen same-entry partial-runner cohort into path-fleet."""
...
def _claim(root: Path, worker: str) -> int:
    ...
    if row is None or row["status"] not in {
        "ADAPTER_REQUIRED", "READY", "SCREENED"
    }:
    ...
    con.execute(
        ...
        (
            worker, now, now,
            "compiled same-entry 192-setting partial cohort ingesting",
...
def _comparison(winner: dict[str, Any], key: str) -> float:
    strategy = float(
        winner["nested"]["validation"]["capital_return_pct"]
    )
    return strategy - float(winner["nested"][key])


def _ingest(
    root: Path, job_id: int, cohort_row: dict[str, Any]
) -> dict[str, Any]:
    if cohort_row["status"] != "OK":
        raise RuntimeError(cohort_row.get("stderr", "cohort error"))
    ...
    if winner["family"] != PATH_ID:
        raise RuntimeError(f"{result['symbol']}: wrong frozen family")
    ...
    payload = {
        ...
        "technical_exit_fills": validation["technical_exit_fills"],
        "reclaim_obligations_created": validation[
            "reclaim_obligations_created"
        ],
        "reclaim_obligations_filled": validation[
            "reclaim_obligations_filled"
        ],
        "reclaim_obligations_unfilled_at_end": validation[
            "clip_obligations_unfilled_at_end"
        ],
        "unfilled_obligation_notional_usd": validation[
            "unfilled_obligation_notional_usd"
        ],
    ...
    return {
        ...
        "partial_exit_fills": payload["partial_exit_fills"],
        "realized_partial_pnl_net_usd": payload[
            "realized_partial_pnl_net_usd"
        ],
        "unfilled_obligations": payload[
            "reclaim_obligations_unfilled_at_end"
        ],
...
def main() -> int:
    ...
    parser.add_argument("--worker", default="path-fleet-partial")
    args = parser.parse_args()
    root = args.root.resolve()
    cohort = json.loads(args.cohort_result.resolve().read_text())
    if cohort["candidate_count_per_key"] != 192:
        raise RuntimeError("partial cohort does not contain full 192 grid")
    ...
    summary: dict[str, Any] = {
        ...
        "contract": {
            ...
            "registry_arithmetic": "4x4x3x2x2=192",
            "max_clip_sum": 0.83,
            "same_entry": True,
            "sides_separate": True,
            "completed_htf_only": True,
            "per_clip_reclaim": True,
    ...
    summary["exact_replay_queue"] = [
        f"{row['symbol']}_{row['side']}" for row in summary["symbols"]
    ...
    fleet.finish(
        ...
        (
            f"partial-runner keys={len(summary['symbols'])}; errors={errors}; "
