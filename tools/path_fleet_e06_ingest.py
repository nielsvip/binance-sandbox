#!/usr/bin/env python3
"""Ingest frozen E06 same-entry validation winners into path-fleet."""
...
PATH_ID = "EXIT_E06_REGRESSION_RETEST"
...
def _claim(root: Path, worker: str) -> int:
    ...
    con.execute(
        ...
        (
            worker, now, now,
            "E06 same-entry registry screen complete; ingesting validation",
...
def _ingest(root: Path, job_id: int, row: dict[str, Any]) -> dict[str, Any]:
    ...
    fleet.atomic_json(path,payload); fleet.add_result(root,path)
    return {
        "symbol": result["symbol"], "side": result["side"],
        "status": status, "strategy_return_pct": payload[
            "strategy_return_pct"
        ],
...
def main() -> int:
    ...
    parser.add_argument("--worker",default="path-fleet-e06")
    args=parser.parse_args()
    root=args.root.resolve()
    cohort=json.loads(args.cohort_result.resolve().read_text())
    if cohort["candidate_count_per_key"]!=192:
        raise RuntimeError("E06 cohort missing 192-arm registry")
    job_id=_claim(root,args.worker)
    run_dir=root/f"job_{job_id}_{PATH_ID}"
    summary: dict[str,Any]={
        "job_id":job_id,"path_id":PATH_ID,"created_at":fleet.utc_now(),
        "source_cohort":str(args.cohort_result.resolve()),
        "contract":{
            "candidates_per_key":192,"same_entry":True,
            "sides_separate":True,"completed_4h_only":True,
            "registry_rebound_atr":"STALE_NAME_MAPPED_TO_CORR_GATE",
            "live_config_status":"DISCONNECTED_RESEARCH_ONLY",
            "confirmation_semantics":(
                "channel reentry OR first adverse break; no later retest state"
            ),
            "red_range":"effective_corr_gate=1.0 is 960/960 inert",
    ...
    for row in cohort["results"]:
        try: summary["symbols"].append(_ingest(root,job_id,row))
    ...
    summary["completed_at"]=fleet.utc_now();summary["errors"]=errors
    summary["exact_replay_queue"]=[
        f"{row['symbol']}_{row['side']}" for row in summary["symbols"]
        if row["status"]=="VECTOR_SURVIVOR_EXACT_PENDING"
    ]
    path=run_dir/"summary.json";fleet.atomic_json(path,summary)
    fleet.finish(
        root,job_id,args.worker,path,
        f"E06 keys={len(summary['symbols'])}; errors={errors}; "
