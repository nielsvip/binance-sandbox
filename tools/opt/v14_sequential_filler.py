...
def main():
    ...
    # baseline — early gate BEFORE clone to avoid wasting compute on truly missing data (no NPZ → gain None)
    # Do NOT skip on TIM 99% vomit valid=False — user needs finished workbooks on all (ZEC/ETH/DOGE) even with high TIM
    ...
    if baseline_vec.get("gain_pct") is None or int(baseline_vec.get("trades") or 0) == 0:
        print(f"[skip-empty-baseline] {new_symside} baseline no NPZ/gain None {baseline_vec.get('invalid_reason')} — skipping clone/fill to avoid waste", flush=True)
        try:
    ...
    for sheet in sheets:
        ...
        for (r, switch, cand) in rows:
            ...
            # Sum positives per INSTRUCTIONS 2/2a — vector_delta + sum(pos yellows), baseline next row if final pos
            # Collect positives from pending_lbI (yellow per-filter deltas)
            positives = [(hdr, d) for hdr, d in pending_lbI.items() if d > 0]
            total_pos = sum(d for _, d in positives) if positives else 0
            # Compute final_delta as combined evaluation if positives else vector_delta
            final_delta = None
            final_variant = None
            final_vec = None
            if positives or vector_delta_val is not None:
                # Build combined variant with all positive yellows
                combined_variant = dict(cumulative_overrides)
                combined_variant[switch] = cand
                for hdr, d in positives:
                    if '=' in hdr:
                        f, opt_raw = hdr.split('=',1)
                        # parse opt
                        try:
                            opt_val = eval(opt_raw) if opt_raw in ('True','False') else opt_raw
                        except:
                            opt_val = opt_raw
                        combined_variant[f] = opt_val
                # Sanitize and evaluate combined
                try:
                    ...
                    if vec_comb and vec_comb.get("valid"):
                        ...
                        final_delta = vg_c - cumulative_before
                        final_variant = combined_variant
                        final_vec = vec_comb
                        print(f"[COMBINED YELLOWS] {sheet}!{r} {switch}={cand} vector {vector_delta_val:.4f} total_pos {total_pos:.4f} combined {final_delta:.4f} vs {cumulative_before:.4f}", flush=True)
                        if final_delta > 0:
                            best = (final_delta, final_variant, "+".join([h for h,_ in positives]), "+".join([h for h,_ in positives]), "+".join([h for h,_ in positives]), final_vec)
                except Exception as _e:
                    print(f"[combined-err] {sheet}!{r} {_e}", flush=True)
        ...
        try:
            sheet_promoted = [(k,v) for k,v in progress.get("done", {}).items() if v.get("cumulative_after") is not None and v.get("delta",0) > 0]
            if sheet_promoted:
