#!/usr/bin/env python3
"""Run and ingest the standard completed-HTF EXIT_E01_CHANDELIER job."""
...
def main() -> int:
    ...
    ap.add_argument("--worker", default="path-fleet-e01-chandelier")
    args = ap.parse_args()
    ...
    if not cohort_path.exists():
        raise RuntimeError(
            f"E01 cohort failed rc={proc.returncode}: {proc.stderr[-2000:]}"
        )
    ...
    summary = {
        ...
        "contract": {
            "grid": {
                ...
                "atr_mult": [1.5, 2, 2.5, 3, 4],
                "profit_gate_pct": [0, 0.25, 0.5, 1],
            },
            "candidate_count_per_key": 160,
            "same_entry": True,
    ...
    fleet.finish(
        ...
        summary_path,
        (
            f"E01 standard same-entry vector keys={len(summary['symbols'])}; "
