#!/usr/bin/env python3
"""Memory-bounded top-10 LONG/bottom-10 SHORT E05 cohort runner."""
...
def _one(row: dict[str, Any], npz_dir: Path, output: Path) -> dict[str, Any]:
    ...
    proc = subprocess.run(
        [
            sys.executable,
            str(ROOT / "tools" / "vec_same_entry_e05_adapter.py"),
            "--artifact",
            str(artifact),
            "--npz-dir",
            str(npz_dir),
...
def main() -> int:
    ...
    payload = {
        "tier": "VEC_RESEARCH_SAME_ENTRY_E05_COHORT",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "matrix_written": False,
        "promotion_allowed": False,
        "candidate_count_per_key": 108,
