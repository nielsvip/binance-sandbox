...
def generate_sym_chart(sym: str, npz: dict, trades: List[Dict], m: Dict,
    ...
    ax_p.set_title(f"fin | {sym}  pool={m['pool_sharpe']:+.4f}  wr={m['wr_pct']:.1f}%  dd={m['max_dd_pct']:.2f}%  trades={m['trades']}  wins={n_win}/{len(pnl)}  ({days}d)", color="#c9d1d9", fontsize=10, pad=6)
    ...
    out_path = out_dir / f"fin_{sym}_{days}d.png"
