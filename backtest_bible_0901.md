...
* `results` (lowercase, sheet index 2) is the **only** results tab and **must show the new results EVERY TIME a pos delta is achieved** so you give the **REASON** for the pos delta and **all metrics**. Columns: `# | switch | setting | delta | new_gain | new_trades | new_tim | new_dd | new_sharpe | source | notes`. `D>0` required; `D<=0` is recorded in `F`/`G` but **never** added to `results` and never becomes a new baseline. **EVERY pos delta row MUST add a row to `results` with `switch`, `setting`, `delta`, `new_gain (=E)`, `new_trades`, `new_tim%`, `new_dd%`, `new_sharpe`, `source=live==vector`, `reason` (e.g., `BB squeeze 0.1 tightens width → +3.6bp/entry`) — no exceptions. If you compute `F>0` but forget the `results` row, that is a **DEATH PENALTY** — results is the audit trail that the baseline chain is real.** `results` is the template for all future `SYM_SIDE` pilots.
