...
def get_template_for_symside(symside: str) -> Path:
    s = symside.upper()
    base = s.rsplit("_", 1)[0]
    is_crypto = base.endswith(("USDT", "USDC", "USD1", "BUSD", "FDUSD", "TUSD"))
    is_long = s.endswith("_LONG")
    if is_crypto:
        return TEMPLATE_CRYPTO_LONG if is_long else TEMPLATE_CRYPTO_SHORT
    return TEMPLATE_STOCKS_LONG if is_long else TEMPLATE_STOCKS_SHORT

def _is_meta_header(v) -> bool:
    return isinstance(v, str) and v.strip() != "" and "=" not in v

def _yellow_bounds(ws) -> tuple[int, int]:
    """New templates: L/M/N (+GLOBAL O/P) are meta (is_default/AVG DELTA/POS_SYM), yellows follow; trailing meta (Live Location/Vec Hook/Type/Correct Options) ends them."""
    start = 12
...
def clone_template(template: Path, new_symside: str) -> Path:
    ...
    # NEW TEMPLATES (2026-09-23) carry row-shifted formulas (=IF(G21="",E20,..), #REF!, =TEMPLATE_BASELINE_METRICS!B2) and
    # stale yellow values from other syms in C/E:J/yellows — clear them; only real per-row numbers may be written.
...
def main():
    ...
    _tpl_arg = Path(args.template)
    if not _tpl_arg.is_absolute():
        _tpl_arg = ROOT / _tpl_arg
    _tpl_side = get_template_for_symside(new_symside)
    if _tpl_side.exists():
        if _tpl_arg.resolve() != _tpl_side.resolve():
            print(f"[template] {new_symside}: using side template {_tpl_side.name} (arg {args.template})", flush=True)
        args.template = str(_tpl_side)
    elif not _tpl_arg.exists():
        print(f"[FATAL] no template for {new_symside}: {_tpl_side} and {_tpl_arg} missing", file=sys.stderr, flush=True)
        sys.exit(3)
    _ACTIVE_TEMPLATE = Path(args.template)
        ...
        if _xls_prev.exists():
            ...
            for _sheet in SWITCH_SHEETS:
                ...
                for _r in range(3, _ws_prev.max_row + 1):
                    ...
                    # only promoted rows (real numeric G>0) carry C — junk C from broken/template sheets is ignored
    ...
    if prepared is None:
        ...
        if ("ZECUSDC" not in new_symside) and (int(baseline_vec.get("trades") or 0) == 0):
            ...
            if _is_kg_never_skip(switch):
                ...
                try:
                    ...
                    if switch in _cat_set:
                        ...
                        pass  # 2026-09-24: no random skip — every row gets real F/G/L:BI (skips left rows None)
                except:
                    pass
                ...
                if not _is_w15m_seq:
                    try:
                        ...
                        if switch in _cat_set_seq:
                            ...
                            pass  # 2026-09-24: no random skip — every row gets real F/G/L:BI
                    except:
                        pass
                    if switch in disabled_switches:
                        print(f"[SKIP-DISABLED] {switch} never had pos delta, skipping for speed (shuffle)", flush=True)
