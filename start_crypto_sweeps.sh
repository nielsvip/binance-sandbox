...
LAUNCH_TIER() {
  ...
  local _running
  _running=$(pgrep -afc "v8_quick_sweep.*--mode crypto|autonomous_search.*--mode crypto" 2>/dev/null || echo 0)
  if [ "$_running" -ge 2 ]; then
    echo "[CRYPTO] CONCURRENCY_CAP: $_running sweep workers already running (cap=2). Refusing new launch — kill existing first."
    pgrep -af "v8_quick_sweep.*--mode crypto|autonomous_search.*--mode crypto" | head -5
    exit 1
  fi
