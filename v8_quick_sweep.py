...
def build_param_grid_exit_decision_tradier():
    ...
    288 configs. Run with V8_RATE_GUARD_DISABLED=1 (honest baseline ~0.07, below rate guard threshold).
    EARLY_ABORT_ENABLED=False: run all symbols so results are not truncated at 40-sym floor."""
    grid = {
        ...
        # Disable early abort — honest baseline ~0.07 < all floors, truncates every config
...
def build_param_grid_exit_decision_crypto():
    ...
    864 configs. Run with V8_RATE_GUARD_DISABLED=1 (honest baseline ~0.07, below rate guard threshold).
    EARLY_ABORT_ENABLED=False: run all symbols so results are not truncated at 40-sym floor."""
    grid = {
        "WRONG_SIDE_ABS_KILL_ENABLED": [True],
        "WRONG_SIDE_WT_TFS_REQUIRED": [2, 3, 4, 5],
