...
async def initial_fetch_and_ranking(symbols, timeframes=None):
    ...
    try:
        ...
        with _tradier_derived_files_lock():
            ...
            # 2026-08-19 FIX — symbols_inf_* must be identical to symbols_trb_* (generated by tradier_rankings).
            # INF mirrors TRB verbatim (1:1). Optional USDT mapping keeps Binance tradability but preserves count (no erasure).
            # Additional highest-scoring sym_side injections (vector_compare) are appended, deduped — never filtered away.
            try:
                _binance_syms = set()
                try:
                ...
                try:
                    ...
                    if _vec_path.exists():
                        ...
                        for _c in _vec.get("long", []):
                            cc = str(_c).upper()
                            if cc not in _inf_long:
                                _inf_long.append(cc)
                        for _c in _vec.get("short", []):
                            cc = str(_c).upper()
                            if cc not in _inf_short:
                                _inf_short.append(cc)
                except Exception:
                    pass
