...
async def initial_fetch_and_ranking(symbols, timeframes=None):
    ...
    try:
        ...
        with _tradier_derived_files_lock():
            ...
            # 2026-08-17 INF hook — populate inf with pos-gain symbols that are also in TRB or mandatory (USDT if exists in symbols.json). Mirrors vector comparison fractional+0.08% winners.
            try:
                _binance_syms = set(json.load(open(config.BASE_PATH / "symbols.json")))
                _inf_long_cands = []
                _inf_short_cands = []
                # pos-gain = long_eligible from ranking ( tradier vector comparison proxy: sharpe>0 )
                _pos_gain_long = {str(it.get("symbol","")).upper() for it in final_ranking_data_scalars if it.get("long_eligible")}
                _pos_gain_short = {str(it.get("symbol","")).upper() for it in final_ranking_data_scalars if it.get("short_eligible") or it.get("gain_pct",0)>0}
                # fallback if eligibility flags absent: use any positive final_score
                if not _pos_gain_long:
                    _pos_gain_long = {str(it.get("symbol","")).upper() for it in final_ranking_data_scalars if float(it.get("final_score_norm",0) or 0)>0}
                _mandatory_long = list(getattr(config, "TRADIER_MANDATORY_LONG_TRB", [])) if hasattr(config, "TRADIER_MANDATORY_LONG_TRB") else ["MU","SNDK","NVDA","GOOGL","META","MSFT","AAPL","ASML","TSLA","AMZN","MRVL","VLO","INTC","MSTR","IBIT","VT","GLD","SLV","COPX"]
                # handle broken config var (line 155 missing name) — fallback above covers it
                if not _mandatory_long:
                    _mandatory_long = ["MU","SNDK","NVDA","GOOGL","META","MSFT","AAPL","ASML","TSLA","AMZN","MRVL","VLO","INTC","MSTR","IBIT","VT","GLD","SLV","COPX"]
                _mandatory_short = list(getattr(config, "TRADIER_MANDATORY_SHORT_TRB", []))
                _allowed_trb_long = set(symbols_trb_long) | set(_mandatory_long)
                _allowed_trb_short = set(symbols_trb_short) | set(_mandatory_short)
                for _sym in sorted(_pos_gain_long & _allowed_trb_long):
                    for _sfx in ("USDT","USDC"):
                        _cand = _sym + _sfx
                        if _cand in _binance_syms:
                            _inf_long_cands.append(_cand)
                            break
                for _sym in sorted(_pos_gain_short & _allowed_trb_short):
                    for _sfx in ("USDT","USDC"):
                        _cand = _sym + _sfx
                        if _cand in _binance_syms:
                            _inf_short_cands.append(_cand)
                            break
                # also include vector-compare validated winners if file exists
                try:
                    import pathlib as _pl
                    _vec_path = config.BASE_PATH / "data" / "vector_compare_inf_candidates.json"
                    if _vec_path.exists():
                        _vec = json.load(open(_vec_path))
                        for _c in _vec.get("long",[]):
                            if _c in _binance_syms and _c not in _inf_long_cands:
                                _inf_long_cands.append(_c)
                        for _c in _vec.get("short",[]):
                            if _c in _binance_syms and _c not in _inf_short_cands:
                                _inf_short_cands.append(_c)
                except Exception:
                    pass
                if _inf_long_cands:
                    await _save_json_async(config.BASE_PATH / "symbols_inf_long.json", sorted(set(_inf_long_cands)))
                    logger.info(f"[INF_HOOK] symbols_inf_long.json ← {len(_inf_long_cands)} pos-gain∩TRB/mandatory USDT: {_inf_long_cands[:15]}")
                if _inf_short_cands:
                    await _save_json_async(config.BASE_PATH / "symbols_inf_short.json", sorted(set(_inf_short_cands)))
                    logger.info(f"[INF_HOOK] symbols_inf_short.json ← {len(_inf_short_cands)} pos-gain∩TRB/mandatory USDT: {_inf_short_cands[:15]}")
