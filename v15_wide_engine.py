# v15_wide_engine — forked from v12_wide_engine.py 2026-09-07, all live paths present
"""v12_wide_engine (formerly v8_vec_sweep).py — THE primary vectorized backtest engine.
...
# ── v12_wide_engine — the WIDE half of the v12 generation ───────────────────
# Renamed from v8_vec_sweep 2026-08-22 on measured evidence, not tidiness. The v8
# brand is retired; this engine is not, because it is not interchangeable with
# v12_quick_engine and the numbers say so plainly (40 sym_sides, same symbol,
# side, window and NPZ, both scored by the same honest metric code):
#
...
#     seconds per evaluation wide 2.67   quick  0.07
#
# They are two tools, not two versions:
#     QUICK  846 real switch reads, 38x faster — the sweep engine
#     WIDE   875 real switch reads, of which 490 QuickConfig cannot even
#            express and 152 more it declares but never reads
#
# So "wide" and "quick", never a version number implying one supersedes the
# other. A name asserting interchangeability would be false, and acting on that
# assertion is exactly what broke 54 files: a 12-line shim aliasing this module
# to vector_engine supplied 2 of the 16 names its readers import. Eight of the
# missing ones start with an underscore, which `from X import *` can never carry,
# and the other six exist in no successor at all.
#
# Do NOT use this as a parity counterpart — parity is v12_quick_engine (vector)
# vs backtest_v12_engine (live call path).
...
def simulate_one_symbol(
    ...
    *,
    ...
    if _npz_cache is not None:
        npz, ts = _npz_cache
        # FIX 2026-08-21 WINDOW: start_ts was dead when _npz_cache passed (MU 869d vs 365d B&H). Slice to window via start_ts.
        if start_ts is not None:
            try:
                import numpy as _np_w
                _i0_w = int(_np_w.searchsorted(ts, start_ts, side="left"))
                _i0_w = max(0, min(_i0_w, len(ts)-1))
                if _i0_w > 0:
                    npz = {k: v[_i0_w:] if hasattr(v, "__len__") and len(v)==len(ts) else v for k,v in npz.items()}
                    ts = ts[_i0_w:]
            except Exception:
                pass
    else:
    ...
    # REAL-WIRED REENTRY_SMA200_BACKUP_ENABLED: bool = True
    REENTRY_POSITIVE_EXIT_SIZE_MULT: float = 1.25  # REENTRY_SMA200_BACKUP_ENABLED — via vec_paths/reentry_sma200_backup
