...
def get_template_for_symside(symside: str) -> Path:
    ...
    s = symside.upper()
    # FIX 2026-09-24: test the SYMBOL, not "ACTUSDT_SHORT" (endswith USDT was always False -> crypto got STOCKS template)
    is_crypto = s.rsplit("_", 1)[0].endswith(("USDT", "USDC", "USD1", "BUSD", "FDUSD", "TUSD", "DAI"))
...
def _is_npz_irrelevant(name) -> bool:
    n = str(name or "").upper()
    if n.startswith("STDEV_") or n.startswith("BAND_SLOPE_SIZING"):
        return False
    return any(p in n for p in SKIP_SWITCH_PATTERNS) or n.endswith(("_3M", "_5M"))
_ACTIVE_TEMPLATE = None
_BASELINE_E3 = None
_VERSIONED_LAST: dict = {}
...
def _load_filter_dictionary() -> list[dict]:
    ...
    candidates = [p for p in [_ACTIVE_TEMPLATE, TEMPLATE_CRYPTO_LONG, TEMPLATE_CRYPTO_SHORT, TEMPLATE_STOCKS_LONG, TEMPLATE_STOCKS_SHORT, ROOT / "SPREADSHEETS" / "TEMPLATE.xlsx", ROOT / "SPREADSHEETS" / "TEMPLATE_V2_1YR_CONNECTED.xlsx"] if p is not None]
    ws = None
    wb = None
    for p in candidates:
        if p.exists():
...
def _atomic_save(wb, wb_path: Path):
    ...
    # 2026-09-24: one versioned snapshot per 15 min per file (was every save -> 3 GB of 2.5 MB copies, disk 88%)
    if versioned and _tm.time() - _VERSIONED_LAST.get(str(wb_path), 0) < 900:
        versioned = None
    elif versioned:
        _VERSIONED_LAST[str(wb_path)] = _tm.time()
    # USER 2026-09-24: ENTRY_REVERSAL_BOUNCE!E3 = calculated baseline NO MATTER WHAT
    try:
        if _BASELINE_E3 is not None and "ENTRY_REVERSAL_BOUNCE" in wb.sheetnames:
            wb["ENTRY_REVERSAL_BOUNCE"].cell(row=3, column=5).value = float(_BASELINE_E3)
    except Exception:
        pass
    try:
        wb.save(tmp)
...
def _is_meta_header(v) -> bool:
    return isinstance(v, str) and v.strip() != "" and "=" not in v

def _yellow_bounds(ws) -> tuple[int, int]:
    """New templates: L/M/N (+GLOBAL O/P) are meta (is_default/AVG DELTA/POS_SYM) — yellows start after them (col O+);
    trailing meta (Live Location/Vec Hook/Type/Correct Options) ends them."""
    start = 12
    while start <= ws.max_column and _is_meta_header(ws.cell(row=2, column=start).value):
        start += 1
    end = max(ws.max_column, start)
    for c in range(start, ws.max_column + 1):
        if _is_meta_header(ws.cell(row=2, column=c).value):
            end = c - 1
            break
    return start, end
...
def ensure_lbI_headers(wb_path: Path):
    ...
    for sheet in SWITCH_SHEETS:
        ...
        try:
            ...
            pass
        existing = set()
        _y_start, _y_end = _yellow_bounds(ws)
        scan_max = max(ws.max_column, 12)
        stale = []
        for c in range(_y_start, scan_max + 1):
            try:
        ...
        for hdr in headers:
            if hdr in existing:
                continue
            while _is_meta_header(ws.cell(row=2, column=col).value):
                col += 1
            try:
...
def clone_template(template: Path, new_symside: str) -> Path:
    ...
    # NEW TEMPLATES (2026-09-23) carry row-shifted formulas (=IF(G21="",E20,..), #REF!, =TEMPLATE_BASELINE_METRICS!B2)
    # and other syms' values in C/E:J/yellows — clear them; v15_pilot decides every value written, never the template.
    for name in SWITCH_SHEETS:
        ...
        for r in range(3, ws.max_row + 1):
            ...
            for c in range(_y_start, _y_end + 1):
                if ws.cell(row=r, column=c).value is not None:
                    ws.cell(row=r, column=c).value = None
            if _is_npz_irrelevant(ws.cell(row=r, column=1).value):
                ws.cell(row=r, column=10).value = "N/A: not in NPZ (3m/5m/max_usd/multiplier)"
            ...
            for row in ws2.iter_rows():
                for c in row:
                    if isinstance(c.value, str) and "TEMPLATE_BASELINE_METRICS" in c.value:
                        c.value = c.value.replace("TEMPLATE_BASELINE_METRICS", new_baseline)
...
def main():
    ...
    global _ACTIVE_TEMPLATE, _BASELINE_E3
    _tpl_side = get_template_for_symside(new_symside)
    if _tpl_side.exists():
        if Path(args.template).name != _tpl_side.name:
            print(f"[template] {new_symside}: using side template {_tpl_side.name} (arg {args.template})", flush=True)
        args.template = str(_tpl_side)
    _ACTIVE_TEMPLATE = Path(args.template) if Path(args.template).is_absolute() else ROOT / args.template
    if not _load_filter_dictionary():
        print(f"[FATAL] FILTER_DICTIONARY_V2 empty from {_ACTIVE_TEMPLATE} — 0 yellows would be computed, refusing to run", file=sys.stderr, flush=True)
        sys.exit(3)
    print(f"[template] {_ACTIVE_TEMPLATE.name} FILTER_DICTIONARY_V2 rows={len(_load_filter_dictionary())}", flush=True)
    ...
    try:
        ...
        # prefer the highest-gain FINAL for this exact sym_side ({SYM}_{SIDE}_bh..._gainXXpYY_30d_matrix.xlsx)
        import re as _re_best
        _finals = []
        for _fp in OUT_DIR.glob(f"{new_symside}_bh*_gain*_30d_matrix.xlsx"):
            _m = _re_best.search(r"_gain(m?)(\d+)p(\d+)_", _fp.name)
            if _m:
                _finals.append(((-1 if _m.group(1) else 1) * float(f"{_m.group(2)}.{_m.group(3)}"), _fp))
        if _finals:
            _xls_prev = max(_finals)[1]
            print(f"[BEST-prev-xls] {new_symside}: best FINAL {_xls_prev.name}", flush=True)
        if _xls_prev.exists():
            ...
            for _sheet in SWITCH_SHEETS:
                ...
                for _r in range(3, _ws_prev.max_row + 1):
                    ...
                    _g = _ws_prev.cell(row=_r, column=7).value
                    # only promoted rows (real numeric G>0) carry C — junk C from template/broken sheets ignored
                    if not (isinstance(_g, (int, float)) and not isinstance(_g, bool) and _g > 0):
                        continue
                    if _is_npz_irrelevant(_a):
                        continue
    ...
    # 2026-09-24: baseline is ONLY what the engine returns for these overrides — no substitution from old progress/defaults (that was a fake number)
    _BASELINE_E3 = float(baseline_gain)
    ...
    # USER 2026-09-24: ENTRY_REVERSAL_BOUNCE first (E3 = baseline, all deltas depart from it), STDEV_SLOPE_SIZING last;
    # tabs strictly sequential — cycle mode (interleaving tabs via a trimmed duplicate row helper) runs as sequential.
    if not args.sheet:
        if "ENTRY_REVERSAL_BOUNCE" in sheets:
    ...
    if _0914_use_cycle and _0914_cycle_schedule is not None:
        ...
        def _process_0914_row_helper(sheet: str, r: int, switch: str, cand):
            ...
            for idx, (variant, filt, fval, hdr) in enumerate(candidates):
                ...
                if filt is None and (best is None or delta > best[0]): best = (delta, variant, filt, fval, hdr, vec)  # USER 2026-09-24: YELLOW filters apply to THIS switch row ONLY — never promoted into cumulative (only naked switch=cand promotes)
            ...
            # combined pos
            try:
                pos_filters = []  # USER 2026-09-24: yellows are never combined/promoted — per-switch info only
                if pos_filters:
            ...
            if ws_h is not None:
                ...
                if False and _per_yellow_sum > 1e-9:  # USER 2026-09-24: isolated yellow deltas are never summed into the switch delta/E
                ...
                try:
                    ...
                    if delta_best > 1e-9:
                        ...
                        ws_h.cell(row=r, column=3).value = cumulative_overrides.get(switch)  # USER 2026-09-24: C keeps the in-force override (NEG never clears it)
                except: pass
            else:
                delta_best, variant_best, filt_best, fval_best, hdr_best, vec_best = best
            key = f"{sheet}!{r}:{switch}={cand}"
    ...
    for sheet in sheets:
        try:
            ...
            for (r, switch, cand) in rows:
                if _is_npz_irrelevant(switch):
                    print(f"[SKIP-NPZ-IRRELEVANT] {sheet}!{r} {switch} — 3m/5m/max_usd/multiplier not modelled by NPZ", flush=True)
                    continue
                if _is_kg_never_skip(switch):
                ...
                try:
                    ...
                    for idx, (variant, filt, fval, hdr) in enumerate(candidates):
                        ...
                        if filt is None and (best is None or delta > best[0]):  # USER 2026-09-24: YELLOW filters apply to THIS switch row ONLY — never promoted into cumulative (only naked switch=cand promotes)
                    ...
                    if best is None:
                        ...
                        try:
                            if ws_row is not None:
                                ...
                                ws_row.cell(row=r, column=3).value = cumulative_overrides.get(switch)  # USER 2026-09-24: C keeps the in-force override (NEG never clears it)
                                try:
                    ...
                    if delta_best <= 0:
                        ...
                        if abs(float(delta_best)) < 1e-9:
                            ...
                            # 2026-09-24: 0.0 is a REAL measured delta — row is written as NEG (was `continue` -> row left None)
                    ...
                    continue
                # periodic flush for NEG/parity paths as well (every 10 rows)
                try:
                    if r % 10 == 0:
                        _atomic_save(wb_keep, wb_path)
                except Exception:
    ...
    if _elapsed_sym > 3600:
        ...
        print(f"[SLOW >1h] {new_symside} elapsed {_elapsed_sym:.1f}s — publishing anyway (return here discarded completed sheets)", flush=True)
