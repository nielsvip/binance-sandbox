...
def compute_tier_metrics(trades_all):
    ...
    for label, rets in tier_trades.items():
        ...
        sharpe = mean_r / std_r if std_r > 1e-9 else 0.0  # per-trade pool_sharpe (sqrt(ANNUAL_BARS_15M) stripped 2026-04-29 per CLAUDE.md rule 4)
    ...
    if trades_all:
        ...
        sharpe_w = mean_w / std_w if std_w > 1e-9 else 0.0  # per-trade pool_sharpe (sqrt(ANNUAL_BARS_15M) stripped 2026-04-29 per CLAUDE.md rule 4)
