"""run_v24_full_sweep.py — Full universe sweep of v24 (15m pyramid) config.
Run on S1 with all 114 tradier + crypto + held-out window.
"""
...
def run_universe(symbols, mode, cfg, start_date, label, write_history=False):
    ...
    print(f"\n{'='*80}")
    print(f"  {label} | {mode} | start={start_date} | {n_syms} syms | {elapsed:.0f}s")
    print(f"{'='*80}")
    ...
    csv_path = RESULTS_DIR / f"struct_v4_v24_{label}_{ts_label}_per_sym.csv"
    if rows:
        with open(csv_path, "w", newline="") as f:
            w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
            w.writeheader()
            w.writerows(rows)
    summary_path = RESULTS_DIR / f"struct_v4_v24_{label}_{ts_label}_summary.json"
    with open(summary_path, "w") as f:
        json.dump({
            "label": label, "mode": mode, "start": start_date,
