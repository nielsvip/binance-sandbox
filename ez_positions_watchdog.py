...
def _is_realtime_running(account_key: str) -> bool:
    try:
        result = subprocess.run(["pgrep", "-f", f"ez_positions_realtime.py {account_key}"], capture_output=True, text=True, timeout=3)
...
def _start_realtime(account_key: str) -> bool:
    script = BASE_PATH / "ez_positions_realtime.py"
    ...
    try:
        ...
        subprocess.Popen([PYTHON, "-u", str(script), account_key], stdout=log_file, stderr=subprocess.STDOUT, start_new_session=True)
        logger.info(f"[{account_key}] Started ez_positions_realtime.py {account_key}")
        return True
...
def _stop_realtime(account_key: str):
    try:
        subprocess.run(["pkill", "-f", f"ez_positions_realtime.py {account_key}"], capture_output=True, timeout=5)
        logger.info(f"[{account_key}] Stopped ez_positions_realtime.py {account_key}")
    except Exception:
        pass
