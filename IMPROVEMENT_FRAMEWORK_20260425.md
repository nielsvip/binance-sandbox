...
| A1 | Funding rate ingest + score component | **DATA PIPELINE LANDED 2026-04-25** | New script `binance_funding_fetcher.py`. Tested on BTCUSDT (1440 records since 2025-01-01). Cache at `data/funding_cache/{symbol}.json`. NPZ fields `funding_rate_{base_tf}` injected via `_inject_funding_oi()` in backtest_v8_precompute.py (crypto only). Score component wiring in `check_entry_candidates_for_account()` PENDING. Backfill all 50 symbols + NPZ regen required. |
| A2 | Open Interest ingest + score component | **DATA PIPELINE LANDED 2026-04-25** | New script `binance_oi_fetcher.py`. Tested on BTCUSDT (2880 records, 30d). Cache at `data/oi_cache/{symbol}.json`. NPZ fields `oi_{base_tf}`, `oi_value_{base_tf}`, `oi_change_15m_{base_tf}`, `oi_change_1h_{base_tf}` injected. Note: Binance limits OI history to ~30d — must run periodically. Score wiring PENDING. |
| A3 | KC + Squeeze fire entry pathway | **DATA LANDED 2026-04-25** | NPZ fields `kc_upper/mid/lower_{tf}`, `squeeze_{tf}`, `squeeze_fire_{tf}` added to backtest_v8_precompute.py. Live helpers `kc_features()` + `squeeze_features()` in ez_indicators.py. Config switches: SQUEEZE_FIRE_ENABLED=False, SQUEEZE_FIRE_TFS=["1h","4h"], SQUEEZE_FIRE_SCORE_BONUS=20. NPZ regen required. Live entry-score wiring PENDING — sweep first. md5 parity verified MacBook+S1+S2. |
| A4 | WT/MFI divergence detector | **DATA LANDED 2026-04-25** | Pivot-based regular/hidden detector wired. NPZ fields `div_reg_bull/reg_bear/hid_bull/hid_bear_{wt,mfi}_{tf}` (8 per TF × 5 TFs = 40 fields). Distinct from existing structure-based `wt_divergence_{tf}` proxy. Helpers `find_pivots_arr()` + `detect_divergence()` in ez_indicators.py. Smoke test passes on engineered series. NPZ regen required. Score wiring PENDING. |
