#!/usr/bin/env python3
"""
Gold Spread Monitor — options pairs trades on miner/streamer premium vs gold spot.

Six pairs, each with backtest-optimized configs:
  NEM/GDX:  Sharpe 0.83 | WR 79% | +$41.5k | LB=10 Z=1.5 Zx=0.5 DTE=60 (141 trades)
  NEM/GLD:  Sharpe 0.81 | WR 88% | +$22.4k | LB=40 Z=1.5 Zx=0.5 DTE=45
  RGLD/GLD: Sharpe 0.73 | WR 87% | +$30.2k | LB=10 Z=1.5 Zx=0.5 DTE=60
  GDX/GLD:  Sharpe 0.75 | WR 81% | +$17.7k | LB=30 Z=2.0 Zx=0.5 DTE=45
  AEM/GLD:  Sharpe 0.66 | WR 82% | +$15.4k | LB=15 Z=2.5 Zx=0.5 DTE=45
  WPM/GLD:  Sharpe 0.68 | WR 79% | +$14.0k | LB=15 Z=2.0 Zx=0.5 DTE=45

Integration:
  - Called by daily cycle (tradier_options_agent.py --daily, Step 8)
  - Standalone: python gold_spread_monitor.py [--scan | --status | --close]
"""
import asyncio
import json
import logging
...
if not logger.handlers:
    ...
    fh = RotatingFileHandler(log_dir / "gold_spread_monitor.log", maxBytes=5 * 1024 * 1024, backupCount=3, encoding="utf-8")
...
PAIR_CONFIGS = {
    "NEM/GDX": {
        "sym_a": "NEM", "sym_b": "GDX",
        "lookback": 10, "z_entry": 1.5, "z_exit": 0.5,
        "z_stop": 4.5, "max_hold_days": 30,
        "target_dte": (45, 75), "budget_per_leg": 1000,
    },
    "NEM/GLD": {
        "sym_a": "NEM", "sym_b": "GLD",
        "lookback": 40, "z_entry": 1.5, "z_exit": 0.5,
        "z_stop": 4.5, "max_hold_days": 30,
        "target_dte": (30, 60), "budget_per_leg": 1000,
    },
    "RGLD/GLD": {
        "sym_a": "RGLD", "sym_b": "GLD",
        "lookback": 10, "z_entry": 1.5, "z_exit": 0.5,
        "z_stop": 4.5, "max_hold_days": 30,
        "target_dte": (45, 75), "budget_per_leg": 1000,
    },
    ...
    "AEM/GLD": {
        "sym_a": "AEM", "sym_b": "GLD",
        "lookback": 15, "z_entry": 2.5, "z_exit": 0.5,
        "z_stop": 4.5, "max_hold_days": 30,
        "target_dte": (30, 60), "budget_per_leg": 1000,
    },
    "WPM/GLD": {
...
ALL_SYMBOLS = {"NEM", "GLD", "GDX", "AEM", "RGLD", "WPM"}
MAX_SPREAD_PCT = 10.0
MIN_OI = 50
MIN_SCORE = 30
STATE_FILE = BASE_PATH / "data" / "tradier" / "gold_spread_state.json"
...
async def find_pair_options(client: TradierAPIClient, pair_name: str, direction: str, price_a: float, price_b: float) -> dict:
    ...
    for symbol, price, is_call_leg in [
        ...
        if not target_exps:
            continue
        center_dte = (target_dte[0] + target_dte[1]) // 2
        target_exps.sort(key=lambda x: abs(x[1] - center_dte))
        exp, dte = target_exps[0]
        opt_type = "call" if is_call_leg else "put"
        sig_direction = "LONG" if is_call_leg else "SHORT"
        sig = DirectionalSignal(symbol=symbol, direction=sig_direction, conviction=80, price=price, atr_D=price * 0.04, signals={"gold_spread": True})
        ...
        if not candidates:
            ...
            if atm_options:
                ...
                logger.info(f"Gold spread {pair_name} {symbol}: ATM {opt_type} ${best['strike']} {exp} mid=${best['mid']:.2f}")
        else:
            best = candidates[0]
            ...
            logger.info(f"Gold spread {pair_name} {symbol}: SCANNER {opt_type} ${best.strike} {best.expiration} mid=${best.mid:.2f} score={best.score}")
    ...
    for leg_key in sorted(options.keys()):
        ...
        if not dry_run:
            ...
            if "order" in res:
                ...
                logger.info(f"Gold spread {pair_name} {direction} {occ} x{qty} @ ${gtc_price:.2f} ID={order_id}")
...
async def run_scan(args):
    ...
    print(f"\n{'=' * 85}")
    print(f"  GOLD SPREAD MONITOR — {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC")
    print(f"{'=' * 85}")
    zscores = {}
    ...
    if not signals:
        print(f"\n  No new signals across gold pairs")
        save_state(state)
        return
    ...
    try:
        for pair_name, zdata in signals:
            ...
            if placed:
                ...
                logger.warning(f"GOLD SPREAD ENTERED: {pair_name} {direction} z={z:+.2f} {cfg['sym_a']}=${zdata['price_a']:.2f} {cfg['sym_b']}=${zdata['price_b']:.2f}")
    finally:
        await client.close()
    save_state(state)
...
async def run_status(args):
    ...
    print(f"\n  Total closed trades: {len(state.get('history', []))}")
...
def main():
    parser = argparse.ArgumentParser(description="Gold Spread Monitor (NEM/GDX/GLD/AEM/RGLD/WPM)")
