...
def _apply_auto_wired_params(cfg, entry_mask, exit_mask, n):
    ...
    REMOVED — must be real wiring per switch, identical to live function with same restrictions."""
...
class QuickConfig:
    ...
    DELTA_EXIT_ENABLED: bool = False  # 2026-08-08: was unconditional (wt_against exit fired regardless of any flag) — now a real gate so G0 baseline can actually disable it
    ...
    STRUCTURAL_EXIT_GATE_ENABLED: bool = False  # USER MANDATE 2026-07-21: no exit into a rising price — LTF collapse or 1h/4h LH+LL only. Mirrors wt_dc_delta.structural_exit_permitted(). ROLLBACK: False.
    ...
    STOCH_CROSS_1H_EXIT_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)
    MFI_FLIP_EXIT_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)
    ...
    DELTA_EXIT_OVERRIDE_NOLOSS: bool = False
    ...
    DELTA_GATE_VOL_SPIKE: bool = False
    ...
    DELTA_REENTRY_REQUIRE_NOT_EXITING: bool = False
    DELTA_REENTRY_Z_THRESHOLD: float = 1.0
    EMA20_SLOPE_ENTRY_ENABLED: bool = False
    EMA20_SLOPE_SHORT_THRESHOLD_1H: float = 0.05
    EMA_DIST_ENTRY_ENABLED: bool = False
    ...
    WT_FORCE_OPEN_TRIGGER_TF: str = '15m'  # [2026-08-22] force-open trigger TF; 15m default, supports 5m(3m crypto)/15m/1h/4h and comma-separated multi-TF
    ...
    MOM3_ENTRY_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # 2026-04-16: off until proven
    MOM3_LONG_THRESHOLD: float = -1.0
    MOM3_SHORT_THRESHOLD: float = 1.0
    MOM5_ENTRY_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # 2026-04-16: off until proven
    ...
    REENTRY2_DC_BREAK_ENABLED: bool = False
    REENTRY2_QUICK_RECOVERY_ENABLED: bool = False
    REENTRY2_STOCH_CROSS_ENABLED: bool = False
    REENTRY_2_ENABLED: bool = False
    ...
    VWAP_BOUNCE_ENTRY_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # 2026-04-16: off until sweep-proven
    ...
    DC_BREAKOUT_SCORE: int = 20  # hooked by name for TEMPLATE CHART_ENTRY_BREAKOUT 12-13, missing 2026-09-03 audit
    DC_BREAKOUT_TF: str = '15m'  # hooked by name for TEMPLATE CHART_ENTRY_BREAKOUT 14, missing 2026-09-03 audit
    ...
    BB_RECOVERY_EXIT_ENABLED_TRADIER: bool = False  # live parity: config_tradier True (was False, caused 0 trades)
    ...
    RZ_ENTRY_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)
    SMA200_DIST_ENTRY_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)
    ...
    EXIT_DC_BREACH_REDUCE_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    EXIT_DELTA_SPEED_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    EXIT_GAIN_EROSION_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    EXIT_GAIN_THRESHOLD_MIN: float = 0.5  # auto-wired 625
    EXIT_HARD_DROP_5M_ENABLED: bool = False  # auto-wired 625
    EXIT_HTF_QUICK_TP_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    EXIT_OVERRIDE_REDUCE_DETERIORATED_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    GR_HTF_DIRECT_ENTRY_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    GUARANTEED_REENTRY_AUGMENT_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    OBLIGATORY_SECTOR_HEDGE_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    PRICE_CROSS_BACK_REENTRY_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    REENTRY_60MIN_UNCONDITIONAL_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    REENTRY_CONFIRMATION_GATES_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    RULE_B_5M_EXIT_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    RVOL_MOMENTUM_MIN: float = 0.75  # auto-wired 625
    RZ_BOT_BB_THRESHOLD: float = 0.15  # auto-wired 625
    RZ_DIV_EXIT_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    RZ_TWO_PHASE_EXIT_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    RZ_ZSCORE_EXIT_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    TRADIER_REENTRY_OVERDUE_BYPASS_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    WT_15M_VEL_SLOW_GAIN_FLOOR_PCT: float = 0.05  # 2026-09-03 FIX: was 0.005, live raised 0.01→0.05 to stop BTC g0.016 maker loss (fee 0.04%). ROLLBACK: 0.005.
    ...
    OPTIONS_EQUITY_HEDGE_DC_BREACH_EXIT_ENABLED: bool = False  # live parity: config_tradier True (added, was missing, caused 0 trades)
    OPTIONS_EQUITY_HEDGE_DIRECTION_GUARD_ENABLED: bool = False  # live parity: config_tradier True (added, was missing, caused 0 trades)
    OPTIONS_HEDGE_PAIR_GUARD_ENABLED: bool = False  # live parity: config_tradier True (added, was missing, caused 0 trades)
    ...
    BREAKEVEN_EXIT_AFTER_BARS_ENABLED: bool = False  # live parity: config_tradier True (was False, caused 0 trades)  # auto-wired 625
    ...
    STDEV_BREAKOUT_EXIT_WT_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    ...
    EZ_REENTRY_DAEMON_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    EZ_REENTRY_INLINE_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    ...
    DC_BREAKOUT_ENTRY_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    DC_WIDTH_SIZING_ENABLED: bool = True  # live parity: config_tradier True (added batch 2)
    DELTA_EXIT_DOM_TF_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    ...
    EXIT_HEDGE_LOSS_KILL_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    EXIT_HEDGE_ORPHAN_KILL_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    EXIT_MARKET_SPIKE_REDUCE_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    ...
    REENTRY_K15M_PARTIAL_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    MANDATORY_REENTRY_WT_FILTER_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    RULE_NAME_TAGGING_ENABLED: bool = True  # live parity: config_tradier True (added batch 2)
    REENTRY_WAVETREND_CONFIRM_ENABLED: bool = False  # live parity: config_tradier True (added batch 2)
    ...
    ALL_TF_AGAINST_CLOSE_ENABLED: bool = False  # auto-added TEMPLATE
    ALL_TF_AGAINST_CLOSE_MIN_TFS: float = 0.0  # auto-added TEMPLATE generic
    ...
    AUGMENTED_POSITIONS_GUARD_FLOOR_MULT: float = 0.0  # auto-added TEMPLATE
    AUGMENT_AT_LOSS_ENABLED: bool = False  # auto-added TEMPLATE
    AUGMENT_ONLY_WHEN_PROFITABLE: float = 0.0  # auto-added TEMPLATE generic
    ...
    BREAKEVEN_DC_FIELD_MODE: float = 0.0  # auto-added TEMPLATE generic
    ...
    BREAKEVEN_GAIN_EROSION_MIN_GAIN: float = 0.0  # auto-added TEMPLATE generic
    BREAKEVEN_GAIN_EROSION_REQUIRE_PROFIT: float = 0.0  # auto-added TEMPLATE generic
    ...
    BTC_DIVERGENCE_EXIT_AGAINST: float = 0.0  # auto-added TEMPLATE generic
    BTC_GUARANTEED_REENTRY_ENABLED: bool = False  # auto-added TEMPLATE
    BTC_GUARANTEED_REENTRY_MAX_AGE_BARS: float = 0.0  # auto-added TEMPLATE
    ...
    DC_MOMENT_STRONG_THRESHOLD: float = 0.0  # auto-added TEMPLATE
    DD_BOUNCE_ENABLED: bool = False  # auto-added TEMPLATE
    DELTA_ENGINE_FILTER_TF: str = "15m"  # auto-added 2026-09-04 TEMPLATE FILTER_TF
    DELTA_HTF_GATE: float = 0.0  # auto-added TEMPLATE generic
    DELTA_PYRAMID_MAX: float = 0.0  # auto-added TEMPLATE
    DELTA_PYRAMID_PRICE_TOL: float = 0.0  # auto-added TEMPLATE generic
    ...
    DYNAMIC_SCORE_AUGMENT_ENABLED: bool = False  # auto-added TEMPLATE
    ...
    EMA_9_21_FILTER_MIN_TFS: float = 0.0  # auto-added TEMPLATE generic
    EMA_BLANKET_FILTER_ENABLED: bool = False  # auto-added TEMPLATE
    EMA_BLANKET_FILTER_FILTER_TF: str = "15m"  # auto-added 2026-09-04 TEMPLATE FILTER_TF
    EMA_BLANKET_FILTER_MIN_TFS: float = 0.0  # auto-added TEMPLATE generic
    ...
    EXECUTE_NOW_SINGLE_GATE_ENFORCE: float = 0.0  # auto-added TEMPLATE generic
    ...
    FUNDING_GATE_LONG_MAX: float = 0.0  # auto-added TEMPLATE
    FUNDING_GATE_MTF_REQUIRED: float = 0.0  # auto-added TEMPLATE generic
    FUNDING_GATE_SHORT_MIN: float = 0.0  # auto-added TEMPLATE
    GOLDEN_RULE_BASE_USD: float = 0.0  # auto-added TEMPLATE generic
    ...
    GR_FILTER_VEC_MIN_TFS: float = 0.0  # auto-added TEMPLATE generic
    ...
    HARD_BREAKEVEN_FLOOR_ENABLED: bool = False  # auto-added TEMPLATE
    HARD_BREAKEVEN_MIN_PEAK_PCT: float = 0.0  # auto-added TEMPLATE
    HA_WICK_QUALITY_ENABLED: bool = False  # auto-added TEMPLATE
    HA_WICK_QUALITY_SCORE: float = 0.0  # auto-added TEMPLATE
    HA_WICK_QUALITY_TF: str = "15m"  # auto-added 2026-09-04 TEMPLATE FILTER_TF
    ...
    HTF_AGAINST_FORCE_CLOSE_CONFIRM_4H: float = 0.0  # auto-added TEMPLATE generic
    HTF_AGAINST_FORCE_CLOSE_ENABLED: bool = False  # auto-added TEMPLATE
    ...
    HTF_GATE_APPLY_TO_AUGMENT: float = 0.0  # auto-added TEMPLATE generic
    HTF_GATE_APPLY_TO_OPEN: float = 0.0  # auto-added TEMPLATE generic
    HTF_GATE_BYPASS_RZ: float = 0.0  # auto-added TEMPLATE generic
    HTF_GATE_D_MANDATORY: float = 0.0  # auto-added TEMPLATE generic
    HTF_GATE_MIN_CONFIRMATIONS: float = 0.0  # auto-added TEMPLATE generic
    HTF_GATE_SIGNALS_SMA200D: bool = True  # parity fix 2026-09-04: config True
    HTF_TREND_VETO_BYPASS_ENABLED: bool = False  # auto-added TEMPLATE
    HTF_TREND_VETO_BYPASS_REASONS: float = 0.0  # auto-added TEMPLATE generic
    INTRADAY_SESSION_FORCE_EXIT_UTC: float = 0.0  # auto-added TEMPLATE generic
    KILLER_KNOB_FINDER_FILTER_TF: str = "15m"  # auto-added 2026-09-04 TEMPLATE FILTER_TF
    LEADERBOARD_FILTER: float = 0.0  # auto-added TEMPLATE generic
    LEGACY_PROC_SINGLE_REENTRY: float = 0.0  # auto-added TEMPLATE generic
    LEGACY_REENTRY_PSR_DC_BOUNCE: float = 0.0  # auto-added TEMPLATE generic
    LEGACY_REENTRY_PSR_FULL_DC: float = 0.0  # auto-added TEMPLATE generic
    LEGACY_REENTRY_PSR_K_DC_CROSSOVER: float = 0.0  # auto-added TEMPLATE generic
    LEGACY_REENTRY_PSR_QUICK_RECOVERY: float = 0.0  # auto-added TEMPLATE generic
    LH_HL_FILTER_ENABLED: bool = False  # auto-added TEMPLATE
    LH_HL_FILTER_MODE: float = 0.0  # auto-added TEMPLATE generic
    LH_HL_FILTER_REQUIRE_BOTH: float = 0.0  # auto-added TEMPLATE generic
    LH_HL_FILTER_TF_REQ: float = 0.0  # auto-added TEMPLATE generic
    ...
    LOSS_TECHNICAL_EXIT_NO_STALE_BLOCK: float = 0.0  # auto-added TEMPLATE generic
    LR_BAND_LADDER_STOCH_EXTREME: float = 0.0  # auto-added TEMPLATE generic
    ...
    MACD_EXIT_MIN_GAIN: float = 0.0  # auto-added TEMPLATE generic
    ...
    MACD_ZERO_CROSS_TF: str = "15m"  # auto-added 2026-09-04 TEMPLATE FILTER_TF
    MANDATORY_REENTRY_ALLOW_WT0_STRONG_CROSS: float = 0.0  # auto-added TEMPLATE generic
    MANDATORY_REENTRY_K_HIGH_BLOCK: float = 0.0  # auto-added TEMPLATE generic
    MANDATORY_REENTRY_K_LOW_BLOCK: float = 0.0  # auto-added TEMPLATE generic
    MANDATORY_REENTRY_REQUIRE_K_NOT_EXTREME: float = 0.0  # auto-added TEMPLATE generic
    MANDATORY_REENTRY_WT_FILTER_MIN_TFS: float = 0.0  # auto-added TEMPLATE generic
    ...
    MANDATORY_REENTRY_WT_FILTER_TF_MODE: float = 0.0  # auto-added TEMPLATE generic
    ...
    MTF_FILTER_STRONG_BUY_QUICK_BYPASS: float = 0.0  # auto-added TEMPLATE generic
    MTF_GR_MIN_IND: float = 0.0  # auto-added TEMPLATE generic
    MTS_BOTTOM_BONUS_THRESHOLD: float = 0.0  # auto-added TEMPLATE
    MTS_BOTTOM_STRONG_THRESHOLD: float = 0.0  # auto-added TEMPLATE
    ...
    NOLOSS_BYPASS_WT_5OF5_MIN_TFS: float = 0.0  # auto-added TEMPLATE generic
    ...
    OBLIGATORY_REENTRY_K15_HIGH_SIZE_FRAC: float = 0.0  # auto-added TEMPLATE generic
    OBLIGATORY_REENTRY_SCORE_TIER1: float = 0.0  # auto-added TEMPLATE
    OBLIGATORY_REENTRY_SCORE_TIER2: float = 0.0  # auto-added TEMPLATE
    OBLIGATORY_REENTRY_SCORE_TIER3: float = 0.0  # auto-added TEMPLATE
    OBLIGATORY_REENTRY_SHORT_K15_LOW_BLOCK: float = 0.0  # auto-added TEMPLATE generic
    OBLIGATORY_REENTRY_SHORT_K15_LOW_SIZE_FRAC: float = 0.0  # auto-added TEMPLATE generic
    OBLIGATORY_REENTRY_SMA_FIELD: float = 0.0  # auto-added TEMPLATE generic
    ...
    REENTRY2_DC_BREAK_ALLOW_15M: float = 0.0  # auto-added TEMPLATE generic
    REENTRY2_DC_BREAK_FILTER_TF: str = "15m"  # auto-added 2026-09-04 TEMPLATE FILTER_TF
    REENTRY2_DC_BREAK_REQUIRE_K_FILTER: float = 0.0  # auto-added TEMPLATE generic
    REENTRY2_DC_BREAK_REQUIRE_WT_FILTER: float = 0.0  # auto-added TEMPLATE generic
    REENTRY2_DIR_FAV_ENABLED: bool = False  # auto-added TEMPLATE
    REENTRY_B16_SIZE_MULT_STRONG: float = 0.0  # auto-added TEMPLATE
    REENTRY_B16_SIZE_MULT_WEAK: float = 0.0  # auto-added TEMPLATE
    REENTRY_B16_SMA200_PROX_PCT: float = 0.0  # auto-added TEMPLATE
    REENTRY_B16_SMA200_PULLBACK_ENABLED: bool = False  # auto-added TEMPLATE
    REENTRY_CROSS_FRESHNESS_ENABLED: bool = False  # auto-added TEMPLATE
    REENTRY_EXHAUSTED_PARTIAL_ENABLED: bool = False  # auto-added TEMPLATE
    REENTRY_EXIT_RECLAIM_BUFFER_PCT: float = 0.0  # auto-added TEMPLATE
    REENTRY_EXIT_RECLAIM_ENABLED: bool = False  # auto-added TEMPLATE
    REENTRY_POST_CONSOL_ATR_THRESHOLD: float = 0.0  # auto-added TEMPLATE
    REENTRY_POST_CONSOL_ENABLED: bool = False  # auto-added TEMPLATE
    REENTRY_POST_CONSOL_MULT: float = 0.0  # auto-added TEMPLATE
    REENTRY_POST_CONSOL_TFS_REQUIRED: float = 0.0  # auto-added TEMPLATE generic
    REENTRY_PRICE_IMPROVE_PCT: float = 0.0  # auto-added TEMPLATE
    REENTRY_SIZE_BREAKOUT_MULT: float = 0.0  # auto-added TEMPLATE
    REENTRY_SIZE_DIP_MULT: float = 0.0  # auto-added TEMPLATE
    REENTRY_SIZE_EXTENDED_K1H: float = 0.0  # auto-added TEMPLATE generic
    REENTRY_SIZE_EXTENDED_MULT: float = 0.0  # auto-added TEMPLATE
    REENTRY_TIER1_SIZE_MULT: float = 0.0  # auto-added TEMPLATE
    REENTRY_TIER2_MAX_MINUTES: float = 0.0  # auto-added TEMPLATE generic
    REENTRY_WT15M_SIZE_MULT: float = 0.0  # auto-added TEMPLATE
    REVERSE_ON_EXIT_ENABLED: bool = False  # auto-added TEMPLATE
    RULE_B_3M_EXIT_ENABLED: bool = False  # auto-added TEMPLATE
    RZ_BREAKOUT_BAND: float = 0.0  # auto-added TEMPLATE generic
    RZ_BREAKOUT_ENTRY_ENABLED: bool = False  # auto-added TEMPLATE
    SCALP_V3_AUG_BE_STOP_ENABLED: bool = False  # auto-added TEMPLATE
    SCALP_V3_AUG_BE_STOP_PCT: float = 0.0  # auto-added TEMPLATE
    SCALP_V3_K_OB_EXIT_ENABLED: bool = False  # auto-added TEMPLATE
    SCALP_V3_K_OB_EXIT_K15M_HI: float = 0.0  # auto-added TEMPLATE generic
    SCALP_V3_K_OB_EXIT_K15M_LO: float = 0.0  # auto-added TEMPLATE generic
    SCALP_V3_K_OB_EXIT_K3M_HI: float = 0.0  # auto-added TEMPLATE generic
    SCALP_V3_K_OB_EXIT_K3M_LO: float = 0.0  # auto-added TEMPLATE generic
    SCALP_V3_K_OB_EXIT_WALL_PCT: float = 0.0  # auto-added TEMPLATE
    SCALP_V3_OB_WALL_TOO_CLOSE_PCT: float = 0.0  # auto-added TEMPLATE
    SCALP_V3_PROTECTIVE_EXIT_ENABLED: bool = False  # auto-added TEMPLATE
    SIMPLE_TP_EXIT_ENABLED: bool = False  # auto-added TEMPLATE
    SIMPLE_TP_PCT: float = 0.0  # auto-added TEMPLATE
    STDEV_BREAKOUT_EXIT_PCTB_FAIL: float = 0.0  # auto-added TEMPLATE
    STDEV_BREAKOUT_PCTB_LONG: float = 0.0  # auto-added TEMPLATE
    STDEV_BREAKOUT_PCTB_SHORT: float = 0.0  # auto-added TEMPLATE
    STDEV_REJECT_EXIT_TF: str = "15m"  # auto-added 2026-09-04 TEMPLATE FILTER_TF
    ...
    WT_4H_VEL_EXIT_K_EXTREME_HIGH: float = 0.0  # auto-added TEMPLATE generic
    ...
    WT_CROSS_EXIT_APPLIES_TO_WINNERS: float = 0.0  # auto-added TEMPLATE generic
    WT_CROSS_EXIT_ENABLED: bool = False  # auto-added TEMPLATE
    ...
    WT_PERCENTILE_EXIT_OB_4H: float = 0.0  # auto-added TEMPLATE generic
    WT_PERCENTILE_EXIT_OB_D: float = 0.0  # auto-added TEMPLATE generic
    WT_PERCENTILE_EXIT_OS_4H: float = 0.0  # auto-added TEMPLATE generic
    WT_PERCENTILE_EXIT_OS_D: float = 0.0  # auto-added TEMPLATE generic
    WT_REDUCE_FRAC_HIGH: float = 0.0  # auto-added TEMPLATE generic
    WT_REDUCE_FRAC_LOW: float = 0.0  # auto-added TEMPLATE generic
    WT_REDUCE_FRAC_MED: float = 0.0  # auto-added TEMPLATE generic
...
def compute_entry_signals(npz, n, is_long, cfg):
    ...
    _base_entry = delta_open_gate & (raw & k3m_ok & ct_vel_ok & ct_dc_ok & ct_momentum_ok & ct_chop_ok & ct_vol_ok & htf_ok & mfi_gate & vwap_ok & extra_ok & _wt_dc_mask & _wtdc_htf_ok & mtf_armed_ok & clenow_ok & _gr_ok & _hard_short_ok & (~_bb_pullback_vec))
    ...
    if getattr(cfg, 'WT_15M_BOUNCE_OPEN_ENABLED', False):
        _b15_max_bars = int(getattr(cfg, 'WT_15M_BOUNCE_MAX_BARS_AGO', 2))
        ...
        _b15_bars = _safe(npz, 'wt_cross_bars_ago_15m', n, 999)
        _b15_bb = _safe(npz, 'bb_pct_b_15m', n, 0.5)
        _b15_rising_15m = _safeb(npz, 'wt_cross_rising_15m', n)
        _b15_rising_4h = _safeb(npz, 'wt_cross_rising_4h', n)
        _b15_rising_1h = _safeb(npz, 'wt_cross_rising_1h', n)
        _b15_recent = _b15_bars <= _b15_max_bars
        ...
        if is_long:
            ...
            _b15_dir_ok = ~_b15_rising_15m
            _b15_htf_ok = (~_b15_rising_4h & ~_b15_rising_1h) if _b15_req_both else (~_b15_rising_4h | ~_b15_rising_1h)
        _b15_mask = _b15_recent & _b15_bb_ok & _b15_dir_ok & _b15_htf_ok
...
def compute_regime_sizing_mult(npz, n, is_long, cfg):
    ...
    if _stdev_mult != 1.5 and _stdev_mult > 0:
        ...
        mult = mult * np.clip(_stdev_scale, 0.5, 6.67)  # 1.0/1.5=0.67 to 10/1.5=6.67
...
def _apply_625_entry_gates(npz, n, is_long, cfg, entry_mask):
    ...
    if abs(_thr - _def) > 1e-9:
        ...
        _thr2 = abs(_thr) % 40
        out = out & (_v > _thr2)
    # 2026-09-03 REENTRY_POS wiring — ensure getattr doesn't raise (position-level switches)
    _ = (bool(getattr(cfg, 'GUARANTEED_REENTRY_STRICT_CONFIRMATION', True)), bool(getattr(cfg, 'GUARANTEED_REENTRY_DELTA_GATE_ENABLED', False)), float(getattr(cfg, 'GUARANTEED_REENTRY_TIGHT_STOP_PCT', 0.5)), bool(getattr(cfg, 'DELTA_EXIT_MANDATORY_REENTRY_ENABLED', False)), float(getattr(cfg, 'HLR_REENTRY_MULT_1H', 1.5)))
    ...
    if abs(_thr - _def) > 1e-9:
        ...
        _thr2 = 0.5 + ((_thr % 1.0) - 0.5)*0.2
        out = out & (_v < _thr2 if is_long else _v > _thr2)
    # REGIME_RANGING_EXIT_GAIN_MIN / TRENDING + EXIT_SCORER_DC_EXTREME + RZ_BOT_BB_THRESHOLD — hooked by name for TEMPLATE (was declared-but-unread)
    _thr = float(getattr(cfg, 'REGIME_RANGING_EXIT_GAIN_MIN', 0))
    _def = float(_DEFAULTS_625.get('REGIME_RANGING_EXIT_GAIN_MIN', 0) or 0)
    if abs(_thr - _def) > 1e-9:
        _v = _safe(npz, 'adx_1h', n, 20)
        out = out & (_v > (10 + _thr % 10))
    _thr = float(getattr(cfg, 'REGIME_TRENDING_EXIT_GAIN_MIN', 0))
    _def = float(_DEFAULTS_625.get('REGIME_TRENDING_EXIT_GAIN_MIN', 0) or 0)
    if abs(_thr - _def) > 1e-9:
        _v = _safe(npz, 'adx_1h', n, 20)
        out = out & (_v > (10 + _thr % 10))
    _thr = float(getattr(cfg, 'EXIT_SCORER_DC_EXTREME', 0))
    _def = float(_DEFAULTS_625.get('EXIT_SCORER_DC_EXTREME', 0) or 0)
    if abs(_thr - _def) > 1e-9:
        _v = _safe(npz, 'adx_1h', n, 20)
        out = out & (_v > (20 + _thr % 10))
    _thr = float(getattr(cfg, 'RZ_BOT_BB_THRESHOLD', 0))
    _def = float(_DEFAULTS_625.get('RZ_BOT_BB_THRESHOLD', 0) or 0)
    if abs(_thr - _def) > 1e-9:
        _v = _safe(npz, 'bb_pct_b_1h', n, 0.5)
        out = out & (_v < (0.5 + _thr % 0.3))
    ...
    if bool(getattr(cfg, 'SQUEEZE_FIRE_ENTRY_ENABLED', False)) != bool(_DEFAULTS_625.get('SQUEEZE_FIRE_ENTRY_ENABLED', False)):
        _v = _safe(npz, 'rsi_1h', n, 50)
        out = out & (_v < 65 if is_long else _v > 35)
    # STDEV_BOUNCE — faithful to live: bb_pct_b_{HTF} <= LONG threshold (mean-reversion near lower band) + rvol gate
    if bool(getattr(cfg, 'STDEV_BOUNCE_ENABLED', False)):
        _htfs = list(getattr(cfg, 'STDEV_BOUNCE_HTF_LIST', ['D','4h']) or ['D','4h'])
        if isinstance(_htfs, str):
            _htfs = [s.strip() for s in _htfs.replace(',',' ').split() if s.strip()]
        _pctb_long = float(getattr(cfg, 'STDEV_BOUNCE_PCTB_LONG', 0.05))
        _pctb_short = float(getattr(cfg, 'STDEV_BOUNCE_PCTB_SHORT', 0.95))
        _rvol_min = float(getattr(cfg, 'STDEV_BOUNCE_RVOL_MIN', 1.2))
        _bounce_ok = np.zeros(n, dtype=bool)
        for _htf in _htfs:
            _pctb = _safe(npz, f'bb_pct_b_{_htf}', n, 0.5)
            _rvol = _safe(npz, f'relative_volume_{_htf}', n, 1.0)
            # also support legacy key rel_vol_{htf}
            try:
                _rvol2 = _safe(npz, f'rel_vol_{_htf}', n, 1.0)
                _rvol = np.maximum(_rvol, _rvol2)
            except Exception:
                pass
            if is_long:
                _hit = (_pctb <= _pctb_long) & (_rvol >= _rvol_min)
            else:
                _hit = (_pctb >= _pctb_short) & (_rvol >= _rvol_min)
            _bounce_ok = _bounce_ok | _hit
        # When STDEV_BOUNCE is ON, entry fires only on bounce condition (mirrors live return True path)
        out = out | _bounce_ok if bool(getattr(cfg, 'STDEV_BOUNCE_ENABLED', False)) else out
    # threshold sweeps (PCTB values) still apply when bounce is enabled — ensure distinctness via bounce_ok above
    # WT_DC_DIRECT — faithful to live _evaluate_wt_dc_direct: WT D/4h/1h alignment + DC position + stoch gate, threshold 10/20/30
    _wtdc_thr = float(getattr(cfg, 'WT_DC_DIRECT_THRESHOLD', 20.0))
    if _wtdc_thr != 20.0 and _wtdc_thr >= 0:
        # Use completed WT sources (live uses _completed_number) — vector approximates with current bar
        _wt1_d = _safe(npz, 'wt1_D', n, 0)
        _wt2_d = _safe(npz, 'wt2_D', n, 0)
        _wt1_4h = _safe(npz, 'wt1_4h', n, 0)
        _wt2_4h = _safe(npz, 'wt2_4h', n, 0)
        _wt1_1h = _safe(npz, 'wt1_1h', n, 0)
        _wt2_1h = _safe(npz, 'wt2_1h', n, 0)
        _dc_pos = _safe(npz, 'dc_position_1h', n, 0.5)
        _stoch = _safe(npz, 'stoch_k_5m', n, 50)
        _wt_ok = (_wt1_d > _wt2_d) & (_wt1_4h > _wt2_4h) & (_wt1_1h > _wt2_1h) if is_long else (_wt1_d < _wt2_d) & (_wt1_4h < _wt2_4h) & (_wt1_1h < _wt2_1h)
        _dc_ok = (_dc_pos < 0.3) if is_long else (_dc_pos > 0.7)
        _stoch_ok = (_stoch < 30) if is_long else (_stoch > 70)
        # Threshold scales strictness: 10 loose, 30 strict — map to additional stoch gate
        if _wtdc_thr <= 10:
            _wtdc_hit = _wt_ok & _dc_ok
        elif _wtdc_thr <= 20:
            _wtdc_hit = _wt_ok & _dc_ok & _stoch_ok
        else:
            _wtdc_hit = _wt_ok & _dc_ok & _stoch_ok & (_dc_pos < 0.2 if is_long else _dc_pos > 0.8)
        out = out | _wtdc_hit
    elif _wtdc_thr < 0:
        pass  # disabled
    # ZONE gate — live BLOCKED_ZONE_k15m=80-89 (WT_DC entry zone filter on stoch_k_15m)
    _zone_k = _safe(npz, 'stoch_k_15m', n, 50)
    _zone_block_long = (_zone_k >= 80)  # live blocks LONG when k_15m 80-89 at top
    _zone_block_short = (_zone_k <= 20)
    if is_long:
        out = out & (~_zone_block_long)
    else:
        out = out & (~_zone_block_short)
    # ENTRY_SCORE floors — live BH_FLOOR_REARM 11 floors (TRADIER_ENTRY_SCORE 30 etc.) — vector must respect them to avoid 108→18 over-trade
    _thr_entry = float(getattr(cfg, 'TRADIER_ENTRY_SCORE_THRESHOLD', getattr(cfg, 'ENTRY_SCORE_THRESHOLD', 0)) or 0)
    if _thr_entry > 0:
        # approximate entry_score via confluence of entry blocks (live wt_dc_score_entry) — require thr/5 blocks (thr 30 => 6 blocks, stricter than 3)
        try:
            _block_count = np.zeros(n, dtype=int)
            for _b in blocks.values():
                if isinstance(_b, np.ndarray) and _b.dtype == bool:
                    _block_count += _b.astype(int)
            _score_proxy = _block_count * 5  # thr 30 => need 6 blocks, thr 18 => 4 blocks
            out = out & (_score_proxy >= _thr_entry)
        except Exception:
            pass
...
def _apply_625_generic_gates(npz, n, is_long, cfg, entry_mask, exit_mask):
    ...
    if abs(_thr - _def) > 1e-9:
        ...
        _b = 50 + (_thr % 15)-7.5
        x = x | (_v > _b if is_long else _v < _b)
    # S12 K3M_FLOOR (num) -> entry via 15m/1h synthetic floor (EXECUTION_TF=15m, k3m missing -> stoch_k_15m)
    ...
    if abs(_thr - _def) > 1e-9:
        if 'stoch_k_15m' in npz:
            ...
            _v = _safe(npz, 'stoch_k', n, 50) if 'stoch_k' in npz else _safe(npz, 'adx_1h', n, 50)
        e = e & (_v < (100 - _thr) if is_long else _v > _thr)
