"""generate_v4_review_mtf.py — Multi-timeframe trade charts for struct_v4.

Shows 4h, 1h, 15m panels per symbol with:
- OHLC bars colored by direction
- Green shading when in position
- Entry (blue line), Exit (red/green line), Pyramid (small dots)
- PnL labels on exit
"""
...
def _build_trade_periods(events):
    """Build list of (open_ts, close_ts, entry_px, exit_px, pnl_pct, n_pyramids)."""
    periods = []
    cur_open_ts = None
    cur_entry_px = None
    cur_pyramids = 0
...
def _plot_ohlc_panel(ax, ohlc, trade_periods, tf_label, max_bars=600):
    """Plot a single TF panel with OHLC bars + trade shading."""
    o, h, l, c, ts = ohlc["o"], ohlc["h"], ohlc["l"], ohlc["c"], ohlc["ts"]
    n = len(o)
    # Downsample if needed
    if n > max_bars:
        step = n // max_bars
        o, h, l, c, ts = o[::step], h[::step], l[::step], c[::step], ts[::step]
        # Re-aggregate highs/lows across skipped bars
        n_new = len(o)
        for i in range(n_new):
            src_start = i * step
            src_end = min(src_start + step, n)
            h[i] = ohlc["h"][src_start:src_end].max()
            l[i] = ohlc["l"][src_start:src_end].min()
        n = n_new

    dates = [datetime.datetime.fromtimestamp(t, tz=datetime.timezone.utc) for t in ts]

    # Plot OHLC as colored bars
    for i in range(n):
    ...
    for tp in trade_periods:
        ...
        # Entry line
        ax.axvline(open_dt, color="#58a6ff", linewidth=0.8, alpha=0.7, linestyle="--", zorder=3)
        # Exit line
...
def make_mtf_chart(sym: str, mode: str, events: list, start_ts: int) -> str:
    ...
    # Build OHLC at each TF from base 5m data
    # 5m = step 1, 15m = step 3, 1h = step 12, 4h = step 48
    tfs = [
        ("4h", 48, 400),
        ("1h", 12, 500),
        ("15m", 3, 500),
    ...
    fig.suptitle(f"{sym}  |  {n_trades} trades  |  WR {wr:.0f}%  |  "
...
def generate_html(acct: str, mode: str, start_date: str, out_path: str,
    ...
    html = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Struct V4 MTF Review — {acct}</title>
<style>
...
<h1>Struct V4 "{acct}" — Multi-TF Trade Review</h1>
<div class="summary">
  ...
  <div class="stat"><div class="big">{np.median([r['ratio'] for r in rows]):.2f}x</div><div class="label">Median xBH</div></div>
</div>
<div class="legend">
  <span><span class="dot" style="background:#58a6ff"></span> Entry (blue dashed)</span>
  <span><span class="dot" style="background:#3fb950"></span> Profitable exit (green dashed)</span>
  <span><span class="dot" style="background:#f85149"></span> Loss exit (red dashed)</span>
  <span><span class="dot" style="background:#0d4420"></span> In-position (green shade = profit)</span>
  <span><span class="dot" style="background:#4d1114"></span> In-position (red shade = loss)</span>
</div>
<h2>Full table — sorted by xBH (worst first)</h2>
<table>
<tr><th>Symbol</th><th>BHx</th><th>Compx</th><th>xBH</th><th>Trades</th><th>Pyramids</th><th>WR%</th><th>Avg%/tr</th><th>Best%</th><th>Worst%</th></tr>
"""
    for r in rows:
        ratio = r["ratio"]
        if r["compound"] < 0.5:
    ...
    for i, r in enumerate(chart_rows):
        section_label = ""
        if i == 0:
            section_label = "<h2 style='color:#f85149'>WORST PERFORMERS</h2>"
        elif i == 10:
            section_label = "<h2 style='color:#d29922'>MIDDLE OF PACK</h2>"
        elif i == 20:
            section_label = "<h2 style='color:#3fb950'>BEST PERFORMERS</h2>"
        html += section_label
        ...
        meta = (f"<span>Trades={r['trades']}</span>"
                ...
                f"<span>B&H={r['bh_mult']:.2f}x</span>"
...
if __name__ == "__main__":
    ...
    ap.add_argument("--acct", default="v24_tradier")
    ap.add_argument("--mode", default="tradier")
    ap.add_argument("--start", default="2024-04-01")
    ap.add_argument("--out", default="charts/struct_v4_review/v24_mtf.html")
