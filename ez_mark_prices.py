...
class MarkPriceStreamer:
    ...
    async def _websocket_connection_handler(self, stream_batch: List[str], batch_id: int):
        """Handles one resilient WebSocket connection for a batch of streams.
        Mirrors ez_prices_ws.py pattern: combined stream URL + async-for iteration + ping_interval=20."""
        ...
        while True:
            try:
                logger.info(f"{log_prefix} Attempting to connect ({len(stream_batch)} streams)...")
                async with websockets.connect(
                    ...
                    async for raw in ws:
                        try:
                            ...
                            logger.debug(f"{log_prefix} Msg error: {e}")
            except Exception as e:
                logger.warning(f"{log_prefix} ⚠️ WS error: {e} — reconnect in {reconnect_delay}s")
