...
rsync -av --include="*.xlsx" --include="*.log" --include="phase4_strategy_results.json" --include="MASTER_SUMMARY.json" --exclude="phase1_*.json" --exclude="phase2_*.json" --exclude="phase4_all_trades.json" niels@157.180.125.52:/home/niels/binance-sandbox/sweep_results/ ~/Desktop/results/ >> "$LOG" 2>&1
